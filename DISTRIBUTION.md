...
- **Not an option: JS TypeScript.** v4 is built around out-of-process tsgo over the mirror faces; reverting to v3's in-process `LanguageService` (JS TypeScript) to sidestep the native binary would give up the performance that motivated the tsgo move (the `rip check` crossover, measured in the audit notes). The type engine is fixed; the platform question is solved within the tsgo model.
