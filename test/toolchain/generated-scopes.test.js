...
const INVENTORY = [
  ...
  { sig: '(params, query) =>', count: 1,
    site: 'render-gate static descriptor key functions',
  ...
  { sig: "this.b.emit(') => ');", count: 1,
    site: 'keyed render reconciliation callback (arrow tail)',
    policy: 'the key expression is covered by render-wide findRenderControl before the synchronous callback emits' },
  { sig: '((${p}) =>', count: 2,
...
const LITERAL_CLASSIFICATION = [
  ...
  generated("') => '", 1, 'keyed reconciliation callback arrow tail; the key expression is validated before emission'),
  ...
  generated("`{ path: '${…}', key: (params, query) => `", 1, 'gate analysis admits only a literal or params/query member path'),
