...
const INVENTORY = [
  ...
  // Ref name is emitReadName (exact `read`); the arrow is this fragment.
  { sig: "`(this._refCleanups ??= []).push(() => ${this.runtimeName('__detachRef')}(this.`", count: 1,
    site: 'render ref cleanup registration',
    policy: 'emitter-owned statement only — no user expression embeds' },
  // Event name is emitQuotedDomName (exact `read`); the arrow is this fragment.
  { sig: "this.b.emit(`, (${ev}) => ${this.runtimeName('__batch')}(() => (`);", count: 1,
...
const LITERAL_CLASSIFICATION = [
  ...
  generated("`, (${…}) => { `", 1, 'binding target control is validated before listener emission'),
  generated("`, (${…}) => ${…}(() => `", 1, 'render handler control is validated before listener emission'),
