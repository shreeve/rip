...
const INVENTORY = [
  ...
  { sig: "this.runtimeName('__computed')}(() => ", count: 2,
    site: 'component computed member (_init and hmr `_hmrRefreshComputeds`)',
    policy: 'computedBody applies the same yield/await rejections as the top-level computed lowering; the hmr twin refreshes bodies on patch without re-running _init' },
  { sig: "key: (params", count: 1,
    site: 'render-gate static descriptor key functions (head; params/query take TS-only face `any` — minted scaffold, the tsScaffoldAny doctrine)',
    policy: 'pure gate analysis admits only literals or params/query member paths; the renderer evaluates each key exactly once' },
  { sig: "this.runtimeName('__effect')}(() => { ", count: 1,
    site: 'render updater effect',
    policy: 'render-wide: renderSyncGuard rejects await/yield where it emits in a render scope (scope depth equals the depth the render opened at); function values push their own scope and pass through' },
  { sig: "this.runtimeName('__effect')}(() => {\\n", count: 2,
    site: 'render if-block and for-block reconciler effects',
    policy: 'emitter-owned statements; embedded user expressions are covered by renderSyncGuard' },
  { sig: "this.runtimeName('__batch')}(() => {\\n", count: 2,
    site: 'render reconciler batch windows (if-block; detach path)',
    policy: 'emitter-owned statements; embedded user expressions are covered by renderSyncGuard' },
  { sig: "if (hasRef) this.b.emit(`${this.runtimeName('__batch')}(() => `);", count: 1,
    site: 'render for-block batch window',
    policy: 'emitter-owned statements; embedded user expressions are covered by renderSyncGuard' },
  { sig: "this.runtimeName('__ownerFrame')}().add(() =>", count: 2,
  ...
  { sig: "if (${instVar}) ${target}.addEventListener('${event}', (${ev}", count: 1,
    ...
    policy: 'handler expressions are covered by renderSyncGuard; function handlers pass through with their own context' },
  { sig: ") => ${this.runtimeName('__batch')}(() => (`);", count: 1,
    site: 'child-component event prop listener (arrow tail)',
    policy: 'handler expressions are covered by renderSyncGuard; function handlers pass through with their own context' },
  { sig: "${el}.addEventListener('${eventName}', (${ev}", count: 1,
    site: 'element event listener (head; the wrapper param takes a TS-only `: any` before the arrow tail)',
    policy: 'handler expressions are covered by renderSyncGuard; function handlers pass through with their own context' },
  { sig: ") => ${this.runtimeName('__batch')}(() => `);", count: 1,
    site: 'element event listener (arrow tail)',
    policy: 'handler expressions are covered by renderSyncGuard; function handlers pass through with their own context' },
  { sig: "${el}.addEventListener('${event}', (${ev}", count: 1,
    site: 'two-way binding write-back listener (head; the param takes a TS-only `: any` before the arrow tail)',
    policy: 'binding targets are covered by renderSyncGuard' },
  { sig: "this.b.emit(') => { ');", count: 1,
    site: 'two-way binding write-back listener (arrow tail)',
    policy: 'binding targets are covered by renderSyncGuard' },
  { sig: '() => this.b.emit(itemVar));', count: 1,
    site: 'keyed render reconciliation callback (head; item/index params take TS-only face types; the item parameter marks the loop head\'s declaration)',
    policy: 'the key expression is covered by render-wide renderSyncGuard as the synchronous callback emits' },
  { sig: "this.b.emit(') => ');", count: 2,
    site: 'keyed render reconciliation callback (arrow tail); render-gate key-fn arrow tail',
    policy: 'the key expression is covered by render-wide renderSyncGuard (reconciliation) / pure gate analysis (gate key) as the synchronous callback emits' },
  { sig: '((${p}) =>', count: 2,
  ...
  { sig: 'static MEMBER_IN = "((k, c) =>', count: 1,
    ...
    policy: 'emitter-owned helper receives source operands as evaluated call arguments; no source body or control transfer is embedded' },
  { sig: "`${i > 0 ? ',' : ''} ${n}: function (this: `", count: 1,
...
const LITERAL_CLASSIFICATION = [
  ...
  generated("` = ${…}(() => `", 2, 'component computed body (_init + hmr refresh) rejects await and yield'),
  ...
  generated("') => '", 2, 'keyed reconciliation callback and render-gate key-fn arrow tails; each key expression is validated before emission'),
  generated("') => { '", 1, 'binding write-back listener arrow tail; targets are covered by renderSyncGuard'),
  ...
  generated("`${…}${…}(() => {\\n`", 3, 'render control and batch windows are validated before reconciler emission'),
  generated("`${…}${…}().add(() => { for (const ${…} of ${…}.blocks) { try { ${…}.d(true); } catch {} } ${…}.blocks = []; ${…}.keys = []; ${…}.items = []; });\\n`", 1, 'emitter-owned loop cleanup'),
  generated("`${…}${…}().add(() => { if (${…}) { ${…}.d(true); ${…} = null; } });\\n`", 1, 'emitter-owned conditional cleanup'),
  generated("`${…}if (detaching) ${…}(() => {\\n`", 1, 'emitter-owned detach batch window'),
...
describe('generated-scope inventory', () => {
  ...
  test('indirect scope-like literals cannot bypass the output-site collector', () => {
    ...
    // semantic 20: ctorAtFields' arrow-boundary propagation compares a
