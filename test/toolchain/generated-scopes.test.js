...
const INVENTORY = [
  ...
  { sig: "() => ${this.runtimeName('__detachRef')}", count: 1,
    ...
    policy: 'emitter-owned statement only — no user expression embeds' },
  { sig: "if (${instVar}) ${elVar}.addEventListener('${event}', (${ev}) => ${this.runtimeName('__batch')}(() => (", count: 1,
  ...
  { sig: "${el}.addEventListener('${eventName}', (${ev}) => ${this.runtimeName('__batch')}(() => ", count: 1,
...
const LITERAL_CLASSIFICATION = [
  ...
  generated("`${…}.addEventListener('${…}', (${…}) => ${…}(() => `", 1, 'render handler control is validated before listener emission'),
  ...
  generated("`if (${…}) ${…}.addEventListener('${…}', (${…}) => ${…}(() => (`", 1, 'render handler control is validated before listener emission'),
