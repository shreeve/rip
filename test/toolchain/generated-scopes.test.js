...
const INVENTORY = [
  ...
  { sig: "this.b.emit(') => ');", count: 2,
    ...
    policy: 'the key expression is covered by render-wide findRenderControl (reconciliation) / pure gate analysis (gate key) before the synchronous callback emits' },
  { sig: '((${p}) =>', count: 2,
...
const LITERAL_CLASSIFICATION = [
  ...
  excluded("`\\ndeclare function __ripRoute<const T extends (${…})>(s: T): T;\\n`", 1, 'an ambient declaration carries no body — no source control flow can inhabit it; a TypeScript-only region, erased from generated JavaScript'),
