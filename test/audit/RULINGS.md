...
| the name after `ref:` | the state binding, `<ElementType> \| null` — value-first, never the container wrapper | **served** — the cell bytes ride the `__ripRefCell` wrap (a real face position) and the name's span joins the value-first channel; pinned as served, gated: hover.ruled |
...
| attr name on an intrinsic (`class:`) | the prop's type, through the tag's receiver surface (`(property) <span>.className: __RipClassValue \| __RipClassValue[]` — the generated surface name reads back as the element) | **served** on the property roads (class/className, value/checked, the innerHTML family), where the key's exact row lands on a real property access; an attribute-road key (`placeholder:`) maps into a typed string-literal position — completions and diagnostics answer there, hover has no symbol and declines; pinned as served, gated: hover.ruled |
...
| member declaration (computed, unannotated) | the same — `(computed) shade: string` | pinned as measured — the face types the member from an INFERRED position (the behavior-object projection is retired), so the value half serves resolved; the minted kind is the open half, like every member row |
