...
export async function roundtrip(wsRoot, files) {
  ...
  const since0 = published.length
  for (const rel of files) {
    versions.set(rel, 1)
    client.notify('textDocument/didOpen', { textDocument: { uri: uriOf(rel), languageId: 'rip', version: 1, text: originals.get(rel) } })
  }
  // Every file's settle runs at once — the waits overlap instead of adding.
  await Promise.all(files.map(async (rel) => baseline.set(rel, (await waitDiags(uriOf(rel), since0)) ?? [])))
  ...
  for (const rel of files) {
    ...
    const uri = uriOf(rel)
    // ── RENAME every top-level declaration. A rename request mutates
    // nothing — the edits come back as a proposal — so a window of them
    // is in flight at once, the way the sweep's hovers are.
    const decls = topLevelDecls(text)
    const WINDOW = 32
    const proposals = []
    for (let w = 0; w < decls.length; w += WINDOW) {
      const chunk = decls.slice(w, w + WINDOW)
      proposals.push(...await Promise.all(chunk.map(async (d) => {
        ...
        return { d, prep, newName, res }
      })))
    }
    for (const { d, prep, newName, res } of proposals) {
