...
function subjectOf(flat) {
  ...
  const head = /^(?:\(alias\) )*(?:\((?:property|parameter|method|accessor|local class|local function|state|prop|computed|readonly|effect|gate|accept|derived|field|bind|key|slot|rest|stash|router)\) )?(?:readonly )?(?:const |let |var |function |import )?/.exec(flat)
  if (head !== null) {
