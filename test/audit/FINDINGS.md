...
| 6 | `@ts-expect-error` dropped on multi-line emit | `tsface`, `check-diagnostics-3a` |
| 7 | No headless `rip check` | `check-usage`, `check-diagnostics-1a` |
...
| 13 | Single-rooted tsconfig — a nested project's own config was ignored | `check-diagnostics-3a`, `editor-gaps-synthetic-project` |
| 14 | Unused `@ts-expect-error` silently swallowed | `check-diagnostics-3a` |
...
| 18 | A directive blinded its whole indented block | `check-diagnostics-3a`'s head-line-only case |
| 19 | Inline render-block directive lost from the face | `check-diagnostics-3a`'s inline component-prop and two-way-bind directive cases; audit `verdict` |
| 20 | Render branch/loop bodies unchecked (`ctx`, loop items) | `check-diagnostics-2b`'s typed-factory-params case; audit `strict` (13-components' render branches and loops) |
...
| 25 | Event handler parameters get no event type | `check-diagnostics-2b`'s handler case; `dom-vocab-lib` |
| 26 | The match operator's emission was never null-clean | `check-diagnostics-1a`, `tsface` |
| 27 | A pattern catch destructured `unknown` | `check-diagnostics-2b`, audit `verdict` (07-exceptions), `tsface` |
...
| 31 | A promoted param declared no field on the checked face | `check-diagnostics-2b` (08-functions) |
...
| 40 | A component member's initializer and in-method writes were never type-checked | `check-diagnostics-1a`, `error-pins.json` (13-components) |
| 41 | A forward-referenced class or component pinned the probe's own symbol — TS2304 on legal code | `check-diagnostics-2b`, `pins` |
| 42 | A wrong-typed schema default was never type-checked | `check-diagnostics-1a`, `error-pins.json` (14-schema) |
...
| 52 | A destructured binding read by a hoisted def was implicitly `any` under strict | `check-diagnostics-3a`, audit `strict` (20-inference) |
...
| 61 | A constructor body's `@field =` declared no field, so every read of it rejected | `check-diagnostics-1a` |
