...
| # | Finding | Tags | Gate |
| --- | --- | --- | --- |
| [21](#21-an-identifier-read-carries-no-source-span) | Identifier reads carry no source span — hover, definition, diagnostics, tokens | `editor`, `compiler` | `census` — **red by design**, the root all four surfaces share; `member` + `use-site` on the token surface |
...
| [53](#53-a-paren-injected-calls-arity-error-lands-on-the-wrong-argument) | A paren-injected call's arity error lands on the wrong argument | `editor`, `compiler` | the Diagnostics Audit (`runner.js`) — 02-operations' paren-less arity row reports its position violation; red by agreement (soft: the audit exits 0), shared with the rewritten-literal row until both close |
...
| [50](#50-a-rewritten-literal-widens-its-neighbours-diagnostics-to-the-whole-element-list) | A rewritten literal widens its neighbours' diagnostics to the whole element list | `compiler` | the Diagnostics Audit (`runner.js`) — 11-types' wrongEntry and wrongTrailing rows report position violations; red by agreement (soft: the audit exits 0), shared with the paren-injected arity row until both close |
...
**Driven, and now standing in the lane** (2026-07-27; the rows entered the fixture 2026-07-29): the two spellings in [02-operations.errors.rip](corpus/errors/02-operations.errors.rip), arguments identical. The explicit `wrongArityParen` asserts clean at the excess argument; the paren-less `wrongArityBare` reports `position TS2554: expected column 32 (count), published 22` — column 22 is the literal `1`.
...
**Status.** ⬜ **Open** (2026-07-27) — gated by the Diagnostics Audit's position row on 02-operations' `wrongArityBare`, red by agreement: the contract carries `diagnostics.positions` with this row's reason beside the rewritten-literal row's, so the run reports the violation and still exits 0 — nothing is pinned as intended, the violation is *reported*, which is what lets the negative stand in the lane before the fix. **The agreement is SHARED with the rewritten-literal row and names both causes, so the invariant clears only when both close.** `diagnostics.positions` reds on ANY position violation, so fixing this row's span alone leaves the other's red standing and the contract's recovery signal — the one that fails when a satisfied reason is left in place — never fires. What DOES speak per row is the lane's own output: this row's exit is `errors/02-operations.errors.rip` reporting zero violations, printed per fixture on every run, and the reason clause naming it must be deleted from `contract.js` in the same change. Both causes are the same root on two list shapes (an injected argument list here, an array/tuple element list there), so one fix plausibly clears both — but they are separate emit sites, and a partial landing is what this paragraph exists to keep honest. **The paren-less spelling is the load-bearing property**: parenthesizing it would position correctly and green the lane while testing nothing, the mitigation this gate must not accept; `wrongArityParen` beside it asserts the explicit spelling clean, so the contrast that isolates the injected cover stays driven. The claims row stays parked ([CLAIMS.md](CLAIMS.md)) until the span lands.
...
**Status.** ⬜ **Open** (2026-07-24) — gated by the Diagnostics Audit's position rows on 11-types' `wrongEntry` and `wrongTrailing`, red by agreement: the contract carries `diagnostics.positions` with this row's reason, so the run reports both violations and still exits 0. **The agreement is SHARED with the paren-injected arity row and names both causes, so the invariant clears only when both close** — it reds on ANY position violation, so narrowing these two spans alone leaves the arity row's red standing and the contract's recovery signal never fires. This row's own exit is `errors/11-types.errors.rip` reporting zero violations in the lane's per-fixture output, with this reason clause deleted from `contract.js` in the same change. **The single-quoted leading literal is the load-bearing property of both fixtures** — rewriting them to double quotes narrows the span and greens the lane with no span work at all, which is the mitigation this gate must not accept. Assigning a whole wrong value instead would green it while testing nothing positional.
