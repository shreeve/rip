...
| [40](#40-a-component-members-initializer-and-in-method-writes-are-never-type-checked) | A component member's initializer and in-method writes are never type-checked | `compiler` | `check`'s component-member case — asserts the silence **as the gap**, both halves; the fix's gate is the member negatives entering 13-components' error pair, and three parked claims rows ([CLAIMS.md](CLAIMS.md)) |
| [42](#42-a-wrong-typed-schema-default-or-transform-is-never-type-checked) | A wrong-typed schema default or transform is never type-checked | `compiler` | `check`'s schema-misdeclaration case — asserts the silence **as the gap**; the fix's gate is the misdeclaration negatives entering 14-schema's error pair |
...
| [27](#27-a-pattern-catch-destructures-unknown) | A pattern catch publishes TS2339/TS2488 from its own lowering | `compiler` | `check`'s pattern-catch case — asserts the TS2339/TS2488 **as the gap**; the corpus parks both spellings ([MANIFEST.md](MANIFEST.md)'s Parked table) until it inverts |
| [31](#31-a-promoted-param-declares-no-field) | A promoted `@`-param declares no field — TS2339 on every member use | `compiler` | `check`'s promoted-param case — asserts the TS2339 pair **as the gap**; 08-functions carries promotion only alongside manual field declarations until it inverts |
...
An identifier READ gets its mapping row from the PrimitiveStore ([stores.js](../../src/stores.js)) — the occurrence span a primitive tree value cannot carry itself, recorded at reduce time and claimed by the emitter as it writes the name. **The reads the census still counts** have no exact row and inherit the **cover** row of whatever construct carries them; each consumer then fails its own way. Three resolvers in [translate.js](../../packages/vscode/src/translate.js) divide the surfaces: hover, definition and tokens ask SOURCE→GENERATED through `sourceOffsetToGeneratedExact`, so a read with no exact row resolves to another symbol or to nothing; diagnostics ask GENERATED→SOURCE through `generatedSpanToSource`, which for a cover row answers with the **whole cover span**, so the squiggle swallows correct neighbours; and the edit path's `generatedEditSpanToSource` refuses outright, returning null rather than guessing.

**Four surfaces, one root** — driven against the real server and tsgo at the argument read in a `console.log` whose label is single-quoted, the shape that collapsed all four:

| surface | source | answer at the read |
| --- | --- | --- |
| hover | `console.log('total:', total)` | `let total: number` — the read's own symbol |
| definition | `console.log('total:', total)` | resolves to the declaration |
| diagnostics | `console.log('total:', totalz)` | underlines `totalz` alone |
| semantic token | `console.log('total:', total)` | delivered at the read |

The diagnostics row needs a *bad* name — a resolving `total` raises nothing to mis-place — so it reads `totalz`; the other three need a resolving one. All four answer from one exact row, which is why no per-surface gate would say more than the census does.

**The diagnostics surface, driven on an ARGUMENT-TYPE rejection** (`rip check` over six spellings of one call, argument at column 7 in every row):

| argument | face | published |
| --- | --- | --- |
| `widen('lit')` | `widen("lit")` — quote rewritten | **6 — the open paren** |
| `widen("lit")` | byte-identical | 7 — the argument |
| `widen(true)` | byte-identical | 7 |
| `widen(word)` | byte-identical | 7 |
| `widen 'lit'` | parens injected | 7 |
| `widen word` | parens injected | 7 |

**Only the quote survives, and only on a LITERAL.** A read is claimed by its own recorded occurrence, so the injected parens no longer displace it and the paren-less spellings position correctly — the trigger that used to fail unconditionally is gone. A single-quoted string literal is not an identifier read: nothing claims it, the rewritten quote breaks the cover's verbatim prefix, and the resolver answers with the cover's left edge — for an explicitly parenthesised call, the `(` one byte left of the argument.

**A third surface, the widest cover in the corpus.** A type error inside a schema callable body used to publish its correct code on the schema's HEAD line, because the block collapses to two parser tokens and the schema rule's annotation records the whole SCHEMA_BODY span as the `body` role. [schema.js](../../src/schema.js) now keeps each word-token occurrence on the descriptor, and the parser carries those spans through the collapse the same way it carries an ordinary list element's — so `bad: ~> @units.toUpperCase()` two lines under a `:shape` marker publishes at `toUpperCase`, not at the marker. This surface mattered most because a schema body is the largest cover the corpus contains, and callables are the only in-body construct that reaches the checker at all — a wrong-typed default or transform publishes nothing (its own row), so every diagnostic a schema body can raise landed on the head line.
...
**The residue is a literal problem, not a read problem.** The one position the diagnostics lane still mis-places is [11-types.errors.rip](corpus/errors/11-types.errors.rip)'s `wrongEntry: Entry = ['a', 'b']`, where the offending element is the string `'b'` and the leading `'a'` rewrites to `"a"`. Its neighbour one line down, `wrongTrailing: Trailing = ['a', label]`, positions correctly — same cover, same rewritten leading literal, but the offending element is a READ and carries its own row. That pair is the whole remaining boundary, and the literal half belongs to #50.
...
- **`use-site`** ([runner.js](runner.js) `faceSurvival`) — count-based, and must be: a dropped token's source offset is unrecoverable, so it does not correspond by position. It takes the SET of names tsgo tokenizes on the face, counts each name's source occurrences, and subtracts what the real server (`session.semanticTokens`) delivered — the deficit is the drop. The server is the reference for delivery, so no remap is reimplemented. It is the only invariant reaching use sites and rip-native names.
...
**The census subsumes `member`, and `member` is now green** — 22 of 22, up from 12. A type-body member with no token is a member with no exact row, so the census counts it from compiler output alone, without a server; with the spans recorded, `member` has nothing left to catch at the mapping layer and now guards token *delivery* — that a name tsgo tokenizes is actually shipped — which the census structurally cannot see. `use-site` was only *root*-subsumed and moved with the root, from 398 drops to 100; its delivery half stays its own.

**The population is the census, not the symptom count.** A read is safe when it sits inside an `exact` row: [builder.js](../../src/builder.js) records `exact` only for verbatim-equal slices, so byte arithmetic *inside* one is valid by construction. Everything else has no positional guarantee — it resolves only while its cover's prefix happens to stay verbatim through it.

| where | reads with no exact row |
| --- | --- |
| the audit corpus — the live number is `--map`'s census line | **5 / 3,472 — 0.14%** |
...
**The corpus rate sat between the two, so for this row it is a fair instrument** — the rate was uniform because the triggers are ordinary syntax, not a shape that needs scale to appear. (It is not fair for #23, where the corpus under-counts by two orders of magnitude and carries the wrong shape.) The two external populations are the reach argument, and re-driving them is what would confirm the corpus rate still generalizes — medlabs is not reachable from this checkout, so that measurement is outstanding. The census exceeds the broken-today subset by design — the remainder resolve only while their cover's prefix holds, which is what the census counts and a symptom gate would not.
...
- **List elements** (call arguments). The list *is* a node with a span — `console.log a, b` yields one node over `a, b` — but per-element spans were never derived: [parser.js](../../src/parser.js) records one row per **role**, and for a spread role `childNodeId` stays null, because [stores.js](../../src/stores.js) takes per-element spans only from *"the children's own NodeStore rows"* and a bare identifier is a primitive string in the tree (`["...", "console", "log"], "total"`), which has none. The span existed transiently at the accumulator's reduce (`locs`) and died when the stack popped — the PrimitiveStore is exactly that span, kept.
- **Annotation names.** The `annotation` role's span is `": number"` — it includes the operator — and `number` gets no node.
- **Type-declaration internals.** `type ID = string` is **one lexer token** (`TYPE_DECL`, via [lexer.js](../../src/lexer.js)'s `claim`) → one node → one role over the whole text. `ID` and `string` had no spans *at all*; the grammar never sees inside a type. This was the largest slice — `string` and `number` topped the at-risk census — and the type-heavy fixtures were the most exposed in the corpus.

**The fix — upstream, and the same move for all three.** Primitive reads carry real source spans in the PrimitiveStore: the lexer's own token offsets, kept alive across each reduce by the rule's carried-value refs and claimed by the emitter as it writes the name, through `markSpan` + `SPAN_ROLES` ([builder.js](../../src/builder.js)) whose `mappingKind` stays decided by the builder's verbatim comparison — so a name row classifies exact **by construction** rather than by assertion. Type internals need no type grammar: the `TYPE` token already carries its span and its text. Schema bodies collapse to one opaque parser token, so [schema.js](../../src/schema.js) hands its own word offsets to the same channel. The store records only under the TS face — the shipping JS emission never queries it and pays nothing for it. **Do not solve this at query time.** A query-time resolver (identity, ordinal, or otherwise) fixes one consumer, cannot serve the edit path — which must never guess — and structurally cannot resolve a name that repeats inside its cover, which an emit-time row resolves for free because the span is *known*, not searched. **A row is worth having only where a name has somewhere to resolve TO** — marking a source word onto emitter-invented scaffolding (`ref` inside `_refCleanups`, `key` inside a reconciler's `keys: []`) satisfies the census with a span that answers nothing, which is the patch the gate exists to refuse. See the upstream rule under [How to read this ledger](#how-to-read-this-ledger).

**The gate this wants is built** — the Mapping Audit ([runner.js](runner.js) `mappingScan`, `bun run audit --map`). Its **census** is exactly this: reads with no exact row, computed straight from the mapping rows, no server and no oracle. It measures the root and nothing else, so no downstream mitigation can satisfy it — only giving reads real source spans reduces it. It is registered as `mapping.census`, **red by agreement** — so the audit is not permitted to be green about it, and, because the contract judges in both directions, the day it empties the run reports `mapping.census RECOVERED` and fails. That is this row's exit, and nothing short of zero reaches it. The same run also reports the broken-**today** subset (`placed`/`text` — reads whose precise resolution refuses or lands on wrong bytes), a strict subset of the census: the remainder resolve only by a verbatim cover prefix, one face rewrite from breaking, which the census counts and a symptom gate would not. The audit's logic was validated against the real editor once and then shipped standalone (no server, no oracle); see [ROADMAP.md](ROADMAP.md) "M1".

**`identity` is what keeps the census honest, and it is zero.** A lower census is not automatically a better one: a mechanism that PAIRS a read to an occurrence rather than knowing it can retire a read from the census by resolving it onto a different occurrence of its own name, which `placed` and `text` both accept because the bytes match. Every remaining read here fails by *refusing* — no exact row, so definition and rename find nothing — never by answering about a neighbour. That is the weaker failure on purpose: the edit path must never guess, so a residue that refuses is a smaller census away from correct, while a residue that guesses is a wrong answer the gate has stopped counting.

**The population is reads that SHOULD resolve.** A word that is syntax in its position resolves to nothing by design — the render DSL's `ref:`, `key:` and `slot`, and a gate's `@app.data` marker, which the lowering erases whole. [RULINGS.md](RULINGS.md) pins those to silence and the hover audit scores them GREEN, so counting them here had two instruments disagreeing about the same bytes. They are excluded, and the exclusion is a table, not a filter: the **compiler** names the kind where it consumes the word — never the audit guessing from spelling, because `key` is a loop variable in [06-loops.rip](corpus/grammar/06-loops.rip) and `ref` a schema field in [14-schema.rip](corpus/grammar/14-schema.rip), and both must keep counting — each kind carries its reason, the count prints every run, and `mapping.exclusions` goes red both ways: on a kind the compiler records that the gate does not declare, and on a declared kind that no longer occurs. Narrowing the population is the one move that could shrink this gate without fixing anything, which is why it is the most policed thing in it.

**What remains — five reads**, read off `--map -v`. **Four cannot take an honest row as things stand**: two annotations their lowering erases outright — a gap parameter's (`(first, …, last: number)` becomes `..._rest` plus a bare `const last`) and a defaulted promoted parameter's (`@level: number = 1` emits `level = 1`, the default inferring it) — and the `offer`/`accept` pair, **parked** in [RULINGS.md](RULINGS.md) with the model unsettled. Giving the first two a row would mean changing the LOWERING to keep the annotation, which is a decision about the face the checker reads, not a mapping fix.

**The fifth is the ordinal claim's own limit, and worth stating as one.** `def choosePort()` declares `chosen`, writes it in both arms of an `if`, then returns it bare. The return's claim opens on the block's `statements` frame, which contains all four occurrences; the three earlier ones were claimed in frames of their OWN, so this frame's cursor is fresh at zero and the already-exact filter does not exclude the declaration — two candidates remain and the first is taken. Nothing about the emission is wrong; the emission simply does not know WHICH occurrence it is. A span the parser recorded per tree position would, which is the difference the fix above names — **known, not searched** — and this read is where the claim is still the searched half.

**A claim only sees its own frame, which is where these went wrong more than once.** The gap parameter's tail binding emits `const last = …` inside the BODY's frame, and the body does not span the parameter list, so the claim could not see the parameter at all until it opened on that parameter's own extent. The same shape, one level up, is why a comprehension's clause variable needed the body's span subtracted. When a read resolves to nothing, the frame is the first thing to check.

**That last read is the ordinal claim's own limit, and worth stating as one.** `def choosePort()` declares `chosen`, writes it in both arms of an `if`, then returns it bare. The return's claim opens on the block's `statements` frame, which contains all four occurrences; the three earlier ones were claimed in frames of their OWN, so this frame's cursor is fresh at zero and the already-exact filter does not exclude the declaration — two candidates remain and the first is taken. Nothing about the emission is wrong; the emission simply does not know WHICH occurrence it is. A span the parser recorded per tree position would, which is the difference the fix above names — **known, not searched** — and the residue is where the claim is still the searched half.

**A node built at EMIT time carries no store row**, which is its own class of miss: `.=` synthesizes the member it re-binds through (`label .= trim()` → `label = label.trim()`), and a constructor's promoted-parameter strip rebuilds each typed-var, so both lost their identity before the mark protocol could see them. The names inside are still source reads and claim their occurrences in the enclosing frame — which is also what keeps the two `string`s in `owner: string` / `constructor: (@owner: string)` apart.

**A re-flow is the same hazard as a reorder**, one axis over. A multi-line import or export clause collapses onto ONE line in the face, so the clause's cover stops being verbatim at the first newline and every name past it loses its position — the names on the opening line survived, which is why the shape read as working. Giving each specifier and each alias its own row cleared the whole file, and took fourteen reads out of *resolving by luck* along with it: they had been surviving on a cover prefix that the next re-flow would have broken.

**A lowering that REORDERS same-named reads is the shape to watch**, and it is the one the census cannot see on its own. A comprehension emits its clause variable ahead of the body that names it, so `(n * 2 for n in nums)` has the two `n`s in opposite orders on the two sides; a claim that walked occurrences in emission order handed the clause variable the body's `n`, byte-equal and wrong. Emission order is not source order, so the claim is bounded by the spans the parser DID record — the `value` role covers the body, each guard is a node with its own extent, and the clause is what neither contains. Where a construct emits a part from a frame wider than the part, that subtraction is what keeps the two apart.
...
- **Type-body members — closed.** `type Circle = { kind: 'circle', radius: number }`: v3 classifies `radius` `property`, and so does v4 — the `member` gauge that tracked this loss is green at 22 of 22.
- **Use sites — closed on the driven shape.** `console.log('total:', total)`: both classify the single-quoted call argument. A probe here must carry a type annotation or padding, because **v3 returns no tokens at all for a file with none** (`lsp.js` gates on `hasTypes`), which on a two-line probe emits nothing and reads as a drop.

Still unsettled for tokens: 08's reactive reads, which dropped in v4 only in render/component context, and that exact context has not been reproduced on v3 — any re-drive must carry a type annotation, or it measures v3's `hasTypes` gate rather than its classifier.

**Hover and definition — v4 now answers, and answers better** (driven both sides, the real servers over stdio at the argument read in `console.log('total:', total)`). v3 hovers **`let total: any`** and resolves the definition; v4 hovers **`let total: number`** and resolves it. The type differs because v3's remap round-trips through its own TS emission while v4 reads the checker's answer at the read's own span — the same permissive inference either way, but v4 keeps the inferred type instead of widening.

**Diagnostics stay unsettled vs v3.** v3's position on a failing read was never driven, and the shape that would settle it — a single-quoted string literal as the offending element — is the one v4 still mis-places.

**Status.** ⬜ **Open** — **gated red by agreement** by the Mapping Audit's census (`bun run audit --map`): reads with no exact row, straight from the compiler output, mitigation-proof. All four surfaces are the same byte-math over the same mapping rows, so the census is the gate for *every* one of them — driven to zero, the cover-collapse mechanism is gone on hover, definition, diagnostics and tokens alike, by construction; no per-surface gate would tell you anything it doesn't. The token surface additionally carries `member` (green) and `use-site`, whose drop count fell with the root and whose delivery half stays its own.

The root fix has landed and the row stays open on the residue, which is the honest reading of a gate whose exit is zero: **the mechanism is right where a read has a recorded occurrence, and what remains is the places one was never recorded or never claimed.** The census fell by an order of magnitude, the diagnostics lane from two mis-positions to one, and the compiled JS is byte-identical across the corpus — the fix records spans only under the TS face, so the shipping path is untouched in bytes and in time. What no gate *drives* is the three server surfaces at a failing read — `verdict` counts Error-severity diagnostics and gets zero (positive fixtures publish nothing by design — their negative content lives in the error lane, so no diagnostic ever sits at a failing read for the mis-position to reach), and the hover audit probes only `declsOf` declarations, never a read — but driving them is a question of server *delivery* (does the plumbing serve the right answer once the spans exist), a concern separate from this finding's root.
...
**Status.** ⬜ **Open** (2026-07-23) — gated as the interim by `check`'s component-member case ([check.test.js](../toolchain/check.test.js)), which asserts the silence **as the gap** for both halves as exact lists: the `:=`/`~=`/`=!` initializers publish nothing beside the plain member's own TS2322, and the three in-method writes (state, prop, plain member) publish nothing beside the method body's TS2304 — the liveness pairs are in-file, so a checker that stopped reporting cannot impersonate the fix. A fix on any form goes red — the cue to invert it and move the member negatives into 13-components' error pair, where the Diagnostics Audit holds them by code and position.
...
**Status.** ⬜ **Open** (2026-07-23) — gated as the interim by `check`'s schema-misdeclaration case ([check.test.js](../toolchain/check.test.js)): the wrong-typed default and transform asserted silent **as the gap**, liveness-paired with a genuine error in the same workspace. It goes red the day the descriptor types either half — the cue to invert it, record which half the fix bought (the fix slot above says they do not close together), and move the misdeclaration negatives into 14-schema's error pair, where the Diagnostics Audit holds them by code and position.
...
**Status.** ⬜ **Open** (2026-07-22) — gated as the interim by `check`'s pattern-catch case ([check.test.js](../toolchain/check.test.js)): the TS2339/TS2488 asserted **as the gap**, codes bound to their lines with columns left free, liveness-paired. It goes red the day the pattern branch mints its annotation — the cue to invert it and move both spellings out of [MANIFEST.md](MANIFEST.md)'s Parked table into 07-exceptions, where `verdict` holds them.
...
**Status.** ⬜ **Open** (2026-07-23) — gated as the interim by `check`'s promoted-param case ([check.test.js](../toolchain/check.test.js)): the TS2339 pair asserted **as the gap** — the promotion line and the member use, codes bound to lines — liveness-paired. It goes red the day the mirror declares the field the promotion implies — the cue to invert it and move the field-less spelling into 08-functions, where `verdict` holds it.
...
`entry: [string, number] = ['a', 'b']` publishes TS2322 spanning `'a', 'b'`, both elements, where TypeScript flags `'b'` alone. The editor squiggle therefore blames an element that is correct, and on a longer tuple the reader is told the list is wrong instead of which slot is.

**The trigger is the OFFENDING element's own quote** (driven, one variable at a time):

| value | flagged span |
| --- | --- |
| `['a', 'b']` | `'a', 'b'` — the whole list |
| `["a", 'b']` | `"a", 'b'` — the whole list |
| `["a", "b"]` | `"b"` — the element |
| `['a', label]` | `label` — the element |

The leading literal is not the variable: double-quoting it changes nothing while the offending element stays single-quoted, and single-quoting it costs nothing once the offending element is double-quoted. What decides the span is whether the element the diagnostic lands on has bytes of its own in the face — a read does, by its recorded occurrence (#21); a re-rendered `'b'` → `"b"` does not, so the resolver falls back to the cover. The fixed-length tuple is incidental. The object literal remains the contrasting case: `{ hits: label }` flags the key, because a key gets its own row. A rest tuple is unaffected — `[string, ...number[]]` reports the whole-value message on the binding, which is TypeScript's own shape for it.

**Why (code) — a mapping fault, not a checker one.** The face is line-identical to the twin (`let entry: [string, number] = ["a", "b"];`), so TypeScript answers the same for both; only the span mapped back to rip source differs.

**The fix is the identifier-read row's move, one value kind over.** Reads are claimed from the PrimitiveStore by their recorded occurrence; a string literal is a primitive too, but the emitter's claim is guarded to identifier-shaped values because a re-rendered literal is not verbatim-equal to its source bytes and would classify `cover` anyway. The span the literal needs is the one the lexer already has — the fix is to record the literal's own source extent and mark the re-rendered text against it, so `mappingKind` stays the builder's verbatim call on a span that is nonetheless correct. v3 narrows correctly (below), so the target span is not in question.
...
**vs v3 — regression, on the literal shape** (driven both sides, 2026-07-28, on `pair: [string, number] = ['a', label]`). v3 flagged `label` alone, TypeScript's own span, where v4 widened to both elements. v4 now narrows on that read shape too, so the surviving regression is the literal one — `['a', 'b']`, not re-driven against v3. Since the face is line-identical on both sides, the fault is in the mapping layer with no appeal to a checker difference.

**Status.** ⬜ **Open** — gated by `diagnostics.positions.element`, red by agreement: the contract carries this row's reason, so the run reports the violation and still exits 0, and fails the day the span narrows without the reason being deleted with it. The invariant is this row's ALONE — the paren-injected arity row has its own — so it recovers on its own evidence: `errors/11-types.errors.rip` reporting zero position violations in the lane's per-fixture output. Its `wrongTrailing` neighbour already positions correctly, and that is what isolates this row to a rewritten literal. **The single-quoted OFFENDING literal is the load-bearing property of the fixture**, not the leading one — double-quoting it narrows the span and greens the lane with no span work at all, which is the mitigation this gate must not accept. Assigning a whole wrong value instead would green it while testing nothing positional.
