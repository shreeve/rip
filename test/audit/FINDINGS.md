...
| [50](#50-a-rewritten-literal-widens-its-neighbours-diagnostics-to-the-whole-element-list) | A rewritten literal widens its neighbours' diagnostics to the whole element list | `compiler` | the Diagnostics Audit (`runner.js`) — 11-types' wrongEntry row reports a position violation; red by agreement (soft: the audit exits 0) until the span maps to the offending element. Its wrongTrailing neighbour now positions correctly, which isolates this row to a rewritten LITERAL: a read in the same slot carries its own span (#21) |
...
**Status.** ⬜ **Open** (2026-07-23) — no gate. A `check` case in the match-operator style — the SILENCE asserted as the gap, liveness-paired with a genuine error in the same workspace — is the honest interim gate, unbuilt. The fix's gate is the misdeclaration negatives entering 14-schema's error pair, where the Diagnostics Audit holds them by code and position.
...
**Driven** (2026-07-27, the Diagnostics Audit over `02-operations.errors.rip`): the two spellings in one file, arguments identical. The explicit call asserts clean. The implicit one reports `position TS2554: expected column 36 (\`count\`), published 26` — column 26 is the literal `1`.
...
**Status.** ⬜ **Open** — gated by the Diagnostics Audit's position row on 11-types' `wrongEntry`, red by agreement: the contract carries `diagnostics.positions` with this row's reason, so the run reports the violation and still exits 0, and fails the day the span narrows without the reason being deleted with it. Its `wrongTrailing` neighbour now positions correctly and is what isolates this row to a rewritten literal. **The single-quoted OFFENDING literal is the load-bearing property of the fixture** — double-quoting it narrows the span and greens the lane with no span work at all, which is the mitigation this gate must not accept. Assigning a whole wrong value instead would green it while testing nothing positional. `contract.js`'s `redBecause` string still carries the tuple framing and wants the same rescoping.
