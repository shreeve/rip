...
| [50](#50-a-rewritten-literal-widens-its-neighbours-diagnostics-to-the-whole-element-list) | A rewritten literal widens its neighbours' diagnostics to the whole element list | `compiler` | the Diagnostics Audit (`runner.js`) — 11-types' wrongEntry and wrongTrailing rows report position violations; red by agreement (soft: the audit exits 0) until the span maps to the offending element |
