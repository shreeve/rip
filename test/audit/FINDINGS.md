...
On linux-x64, once a module's mirror has been pruned to an auto-import stub and re-materialized, every probed binding typed through a **dynamic** import of that module answers `any` and never settles (driven to 30 s; the audit's retry budget is 1.2 s), while darwin answers typed in milliseconds. Minimal repro: open `10-modules-lib.rip`, close it, open `10-modules.rip`, hover `lazyHost`; `import!` wedges identically, and the `import('./10-modules-lib.rip').Parcel` type position joins them under the concurrent probe pool. Tokens degrade at the same positions. The stub wedges *static*-import bindings just as hard — the audit's probe population simply contains none fed by the pruned module, which is why the report shows only the dynamic pair.

**Why (code)** — pinned, and rip-side. The document-compile path in [server.js](../../packages/vscode/src/server.js) calls `materializeClosure(imports)` and discards the returned `touched` list, so mirror files written by an open-driven materialization are never forwarded to tsgo as `workspace/didChangeWatchedFiles` — tsgo's program keeps the bytes it last knew, which after a close-time prune is the stub (`pruneClosure` DOES notify its restub in; the re-materialized real face never follows). The platform split is masking, driven against raw tsgo with no rip server: darwin natively watches the disk (fswatch/FSEvents — an unnotified rewrite recovers in ~250 ms whatever the client advertises), linux does not effectively watch in this arrangement (an unnotified rewrite never recovers), and the notification path itself is sound — one synthesized `Changed` on linux recovers static, dynamic and import-type positions in ~3 ms. Causally confirmed at the seat: forwarding `touched` (one hunk, the shape the watcher-event path already uses) takes the linux churn repro from a 30-second wedge to 8 ms.

**The fix** — forward `touched` from the compile path's materialization, as the watcher-event path forwards it and the prune forwards its restubs; the channel and the change-type mapping already exist in the file. It must NOT lean on tsgo's native watching — darwin-only in practice, and the LSP contract puts event delivery on the client once it advertises `didChangeWatchedFiles`, which the server does ([tsgo.js](../../packages/vscode/src/tsgo.js)). It must NOT be closed by widening the audit's retry budget: without the event the answer never settles. Upstream is a NOTE, not a dependency: tsgo's linux build did no native watching even when the client declined the watch capability, worth reporting but not this row's root. The fix is applied at the seat, and the CI audit job runs on ubuntu ([test.yml](../../.github/workflows/test.yml)) — the platform that proves delivery rather than being rescued by FSEvents.

**Why the suite missed it.** The audit had only ever run on macOS, where tsgo's native watcher delivered the event the server never sent — the defect was masked on every machine that had ever run the instrument. The first ubuntu run of the newly wired CI audit job surfaced it immediately.

**vs v3** — **not drivable in kind:** no mirror tree, no stub/prune cycle, no LSP notification seam — the machinery this finding lives in does not exist at 3.17.5.

**Status.** ⬜ **Open** (2026-08-04) — the gate is wired and pending its first run: the audit contract on ubuntu (the `audit` CI job), whose hover-parity and token-delivery invariants reproduce this wedge unfixed. Its first green run closes this row.
