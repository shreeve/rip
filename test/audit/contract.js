...
export const CONTRACT = [
  ...
  },
  {
    ...
    redBecause: "a rewritten literal earlier in an element list widens its neighbours' diagnostics to the whole list rather than the offending element, so 11-types' wrong-element rows land wide — their single-quoted leading literal is load-bearing, and double-quoting them would narrow the span without fixing anything; delete this when the span lands on the offending element",
  ...
  },
  {
    ...
    redBecause: "a paren-injected call's arity error lands on the first argument rather than the excess one, so 02-operations' paren-less arity row lands early — its paren-less spelling is load-bearing, and parenthesizing it would position correctly while testing nothing; delete this when the span lands on the offending argument",
