...
The audit stays a STANDALONE script, outside `test` and `test:all`; CI gates it as its own job (`audit` in `.github/workflows/test.yml`), so a contract break paints the push that made it. The judgment itself is additionally gated at unit speed by `test/toolchain/audit-contract.test.js`: both directions, one-predicate-per-invariant reachability, a lane that did not run never assumed green, and a refusal when a tolerated red cites a ledger row by number instead of stating its reason.
