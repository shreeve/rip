...
*Built.* `--map`. Walks every source identifier and checks it maps to a generated position holding the same text, from the compiler's own rows. Two invariants partition the failures: `placed` (the precise resolver refuses — a rewrite) and `text` (it resolves to the wrong bytes — the #21 hazard), each classified by the row it fell to and by root. A third, `identity`, closes what byte-equality cannot see: a read resolving onto a DIFFERENT occurrence of its own name satisfies both of the others, so every resolved read is mapped BACK through the editor's own reverse direction and must land on a source span containing the offset it came from. Expected zero, and gated — unlike its two neighbours, which count the open mapping gap. It also proves each pass that no flagged read lacks a containing row. The census counts the AT-RISK population — reads with no exact row, larger than what misleads the editor today, since some resolve only by sitting at their cover's start.
