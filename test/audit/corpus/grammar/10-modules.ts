...
// ── Import attributes: `with { type: 'json' }` after the source — default, namespace, named, and side-effect forms ──

import settings from './10-modules-data.json' with { type: 'json' }
import * as dataModule from './10-modules-data.json' with { type: 'json' }
import { port as dataPort } from './10-modules-data.json' with { type: 'json' }
import './10-modules-data.json' with { type: 'json' }

console.log('attributes:', settings.name, dataModule.default.port, dataPort, settings.tags.length)
