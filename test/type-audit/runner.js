...
// them — the corpus-expected convention). The report also scores every
// probe answering a REAL type (not `any`) — the gauge to keep full.
//
...
// the gauge metric: probes answering a REAL type, not `any` — full is
// the goal, so the ratio reads like every Score row above it.
const typedCount = probeCount - anyCount;
const typedRatio = `${typedCount} / ${probeCount}`;
