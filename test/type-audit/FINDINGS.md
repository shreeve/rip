...
**Within that band the axis is *silently wrong* over *visibly missing*.** #20, #24 and #25 rank first: they let code you wrote go unchecked while the editor looks clean — the worst thing an editor can do. #22 is the most *felt*, wrong at every dot; #8 fails visibly and harmlessly. (Identifier-read span coverage — formerly #21 — is closed behind `census`.)
...
| 21 | Identifier reads carry no source span | `census` (map audit); `member` confirms type-body coverage, `survival` tracks the separate delivery half |
