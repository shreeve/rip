# Type-audit findings — verified gaps in rip's typed-editor story

> Ported from rip-v4 to v5; statuses reflect **v5**, verified by re-driving every finding against v5's compiler and editor server. The pre-fork fixes (#1–#6, #9, #10, C1-face, C2) hold on v5's reworked compiler; C1's `.d.ts` (optional params + generics), #11, and #12 landed in **this PR**. rip-v4 commit hashes and line-anchors were dropped in the port. The **vs v3** comparisons were established by driving v3 (the rip-lang repo) before its retirement — those commands are historical, not runnable in v5.

Each finding's body is the original audit snapshot (hence present-tense, even once fixed); the **Status** line under each heading is the current truth. Evidence tags: **[driven]** = reproduced by running the tool, command cited; **[code]** = verified by reading source, file:line cited.

## Summary

| # | Finding | Class | Status |
| --- | --- | --- | --- |
| [C1](#c1-optional-marker-rejected) | Optional `?` marker rejected | Compiler blocker | ✅ Fixed (verified on v5) |
| [C2](#c2-method-shorthand-in-type-body-rejected) | Method-shorthand in type body rejected | Compiler blocker | ✅ Fixed (verified on v5) |
| [1](#1-implicit-any-suppressed-with-no-opt-out) | Implicit-any suppressed, no opt-out | Silent safety hole | ✅ Fixed (verified on v5) |
| [2](#2-use-before-assign-hidden-on-annotated-forwards) | Use-before-assign hidden by `!` | Silent safety hole | ✅ Fixed (verified on v5) |
| [3](#3-reactive-binding-annotations-not-enforced) | Reactive annotations not enforced | Silent safety hole | ✅ Fixed (verified on v5) |
| [4](#4-evolving-let-reassignment-not-caught) | Evolving-`let` reassignment not caught | Silent safety hole | ✅ Fixed (verified on v5) |
| [5](#5-typeof-on-an-unannotated-value-resolves-to-undefined) | `typeof` unannotated → `undefined` | Loud correctness | ✅ Fixed (verified on v5) |
| [6](#6-ts-expect-error-dropped-on-multi-line-emit) | `@ts-expect-error` dropped on multi-line | Loud correctness | ✅ Fixed (verified on v5) |
| [7](#7-no-headless-type-checker-rip-check) | No headless `rip check` | Missing capability | ⬜ Open |
| [8](#8-auto-import-is-closure-scoped) | Auto-import closure-scoped | Missing capability | ⬜ Open |
| [9](#9-write-only-unannotated-locals-hover-any) | Write-only locals hover `any` | Hover DX | ✅ Fixed (verified on v5) |
| [10](#10-reactive-bindings-hover-as-their-cell-wrapper) | Reactive bindings hover cell wrapper | Hover DX | ✅ Fixed (verified on v5) |
| [11](#11-config-changes-required-a-reload) | Config changes required a reload | Config surface | ✅ Fixed (this PR) |
| [12](#12-nocheck-parsed-but-never-applied) | `rip.noCheck` parsed but never applied | Config surface | ✅ Fixed (this PR) |
...
**Status.** ✅ **Fixed · verified on v5** — face, `.d.ts` optional params, and generics all closed · [driven]

- **Face** ✅ — `bin/rip --ts` on 03/05/06 carries the `?`.
- **`.d.ts` optional params (C1-dts)** ✅ (this PR) — `dts.js` reads the `optionalMarker` role (shared `renderParam`): typed `title?: string` keeps the `?`, bare `title?: any` (a declaration can't carry an implicit any). tsc-valid; matches v3.
- **`.d.ts` generics (generics-dts)** ✅ (this PR) — declarations emit the `<T, …>` clause from the `typeParams` role, so a generic `def` references `T` in scope instead of `TS2304`.
...
**Reproduced** — two surfaces (pre-fix output; both compile now — see Status): type-body members ([03-structural.rip:12](fixtures/03-structural.rip) `ssl?: boolean`, [05-interfaces.rip:23](fixtures/05-interfaces.rip) `method?: string`) and params ([06-functions.rip:40](fixtures/06-functions.rip) `title?: string`).
```
...
**vs v3** — compiles all three (driven, EXIT 0 above) and emits the `?` into the `.d.ts` (`title?: string`), per the fixtures' own comments ([06-functions.rip:39](fixtures/06-functions.rip)).
...
**Status.** ✅ **Fixed · verified** · [driven]
Face and `.d.ts` both render the shorthand; isolated `.d.ts` passes `tsc --strict`. Held on both surfaces because it landed in the shared `typetext.js` seam (which `dts.js` also uses) — the positive control for C1's drift.

**Reproduced** — [09-components.rip:273](fixtures/09-components.rip): `addItem(item: CartItem): void` inside `export type Cart =` (pre-fix output; compiles now).
```
...
**vs v3** — compiles it (EXIT 0) and preserves the shorthand as a real method signature in its `.d.ts` — `addItem(item: CartItem): void;` (driven against v3). The `.tsx` twin ([09-components.tsx:274](fixtures/09-components.tsx)) expresses the same member as a property arrow type (`addItem: (item: CartItem) => void`) — the TS-equivalent, not the shorthand form.

## Regressions vs v3 (v4 behind v3)

v4 is behind v3 here — consequences of the tsgo/LSP broker and the strip-gated face replacing v3's in-process LanguageService and its free-form type-checking shadow. **Ordered most-severe first:** silent, no-opt-out safety holes a strict project can't recover (1–4) → loud correctness breaks on valid typed code (5–6) → missing/degraded capabilities (7–8) → hover DX degradations (9–10). All verified by hand.
...
**Status.** ✅ **Fixed · verified** · [code + driven]
`rip.strict` stops suppressing the implicit-any family. Drove the real LSP server — `TS7006` surfaces on the `.rip` param under `strict:true`, hidden under `strict:false`.

**Reproduced** — `greet = (name) -> name.toUpperCase()` → face `function(name){…}`; `tsc --strict` on the face reports `TS7006: Parameter 'name' implicitly has an 'any' type`, but the v4 editor drops it at [server.js:986](../../packages/vscode/src/server.js) (`if (SUPPRESSED_TS_CODES.has(d.code)) return null;` — the set is [translate.js:29](../../packages/vscode/src/translate.js)).
...
**Status.** ✅ **Fixed · verified on v5** · [driven]
(with #1): under `rip.strict` the emitter drops the `!` on typed forwards, so `TS2454` is checked. Not re-driven here.
...
**Status.** ✅ **Fixed · verified on v5** · [driven]
same declare-in-place campaign as #4: the `typeof`'d binding declares in place, so `typeof` reads its real value type. Not re-driven here.

**Reproduced** — [02-aliases.rip:27](fixtures/02-aliases.rip): `defaults = {theme:'dark',lang:'en'}` / `type Defaults = typeof defaults` / `prefs: Defaults = {…}` → the face fails `tsc --noEmit --strict` with `TS2322: … not assignable to type 'undefined'`; the v4 editor squiggles it.
...
**Status.** ⬜ **Open** · [code]
no `rip check` was built in #83; the finding's own TODO stands.
...
**vs v3** — v3's checker was an in-process LanguageService CLI; v4's moved into tsgo/LSP, so the headless equivalent no longer exists. **TODO (tracked per request):** build a `rip check` — the batch counterpart of what the editor already does (faces → mirror → tsgo batch → map diagnostics back). Not built.
...
**Status.** ⬜ **Open** · [code + driven]
auto-import scope unchanged (no commit in #83).
...
**Why (code)** — the generated tsconfig's `include` is `['**/*.ts']` rooted at the mirror closure ([server.js:315](../../packages/vscode/src/server.js)); the candidate set is exactly the tsgo program.
...
A **non-exported** unannotated local — a value *or* an arrow function (`f = -> …`) — assigned once and never read hovers `any` in v4; v3 showed the inferred type. Mildest of the set: degraded hover in a transient/authoring state (self-heals once any read exists), no hidden bug, no broken code. **The scope is narrow, and verified:** evolving-`let` types every case that HAS a read — including a *cross-scope* closure read (an outer var read inside a nested fn types correctly) — and **exported** bindings are exempt (they emit declare-in-place, `export const f = …`), so a `util.rip` of exported-but-uncalled functions all hover their real types. Only a never-read *local* falls to `any`.

**Status.** ✅ **Fixed · verified on v5** · [code + driven]
declare-in-place () infers has-a-read cases natively; pin-probe the write-only remainder. Not re-driven here.

**Reproduced** — [11-inference.rip:123](fixtures/11-inference.rip): `matches = filterBy('a')` (never read) hovers `let matches: any` in v4 vs `let matches: string[]` in v3 (a hover-differential no-oracle invariant hit).

**Why** — the face hoists to an evolving-`let`: TS types it `any` at the declaration and every write, materializing the real type only at reads. `enrichEvolvingAnyHover` ([server.js:1455](../../packages/vscode/src/server.js)) recovers it by querying a read site — with zero reads there is nothing to query, so `any` stands.
...
**Status.** ✅ **Fixed · verified on v5** · [driven]
the broker rewrites reactive-cell hovers to the value type; makes the runtime generic on the face. Not re-driven here.

**Reproduced** — dim-6 hover differential (v3 oracle) over [08-reactive.rip](fixtures/08-reactive.rip) — four shapes, all driven:
...
## Config surface — rip.strict / rip.noCheck (discovered in #83 review)

Two defects found while verifying finding #1, both landed in this PR: `package.json#rip` config was not reactive, and `rip.noCheck` (v3's `exclude`) was parsed but never applied.
...
**Status.** ✅ **Fixed · verified** · [code + driven]
Added `**/package.json` to the watch globs and a handler branch that refreshes every open doc on a non-`node_modules` `package.json` change — a full refresh, since `strict` changes the face itself and a re-pull alone would miss it. Driven over the real LSP server: flipping `rip.strict` surfaces/hides `TS7006` on an open doc with no reload. [server.js:947](../../packages/vscode/src/server.js).
...
**Status.** ✅ **Fixed · verified** · [code + driven]
Renamed to `rip.noCheck` (the glob form of the per-file `# @ts-nocheck` directive: matched paths stay in-program so imports resolve, but their diagnostics are silenced) and wired it into `refresh()` and `repullDiagnostics`. Type diagnostics for matched paths are suppressed; a genuine compile error still surfaces. Driven over the real LSP server: `noCheck: ['legacy/**']` drops an open file's diagnostics to 0 (reactively) while a sibling still reports. [config.js:29](../../src/config.js).

## `=` hoisting: four type regressions and a bounded fix (owner's call)

**Status.** Addressed
The declare-in-place fix this note proposed shipped as the evolving-`let` tiers (+ follow-ups). #4/#5/#9 and the excess-property case close through it (each `Fixed · verified on v5` above).
...
**The root.** Hoisting splits a plain `=` binding's declaration from its value: `x = 1` → `let x; … x = 1`. Four verified regressions trace to exactly that split — and it cuts *both* ways, so it is not simply "more permissive":
...
- **spurious excess-property error** (over-check, unnumbered) — `def getEl(): {tag:string}` with `o = {tag, __meta}` emits `let o; o = {…}`, and the evolving `let` takes the return type as context, so the fresh literal trips `TS2353` — which `const o = {…}` does not ([fixtures/12-cast.rip:41](fixtures/12-cast.rip), driven: v4's hoisted form errors under `tsc --strict`, the declare-in-place form is clean).
...
Everything the six-dimension runner surfaces is now accounted for above. The remaining raw signal was triaged and adds no new findings:
...
So every finding here is something we have personally driven; nothing remains held out.
