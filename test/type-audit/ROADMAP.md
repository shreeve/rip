...
Depends on M2 (built) and RULINGS.md — the components/schema files and their pinned negatives are gated on it. Produces: the corpus — and M1, M4, and M5 see only constructs the corpus contains, so their completeness is bounded here.
