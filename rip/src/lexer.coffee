...
class Lexer
  ...
  identifierToken: ->
    ...
    if id is 'as' and @seenImport
      ...
      else if @value(yes) in RIP_KEYWORDS
        prev = @prev()
    ...
    tokenData = {}
    if tag is 'IDENTIFIER' and (id in JS_KEYWORDS or id in RIP_KEYWORDS) and
       not (@exportSpecifierList and id in RIP_KEYWORDS)
      tag = id.toUpperCase()
...
isUnassignable = (name, displayName = name) -> switch
  when name in [JS_KEYWORDS..., RIP_KEYWORDS...]
...
RIP_ALIASES  = (key for key of RIP_ALIAS_MAP)
