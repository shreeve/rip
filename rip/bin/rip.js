...
class RipCLI {
  ...
  parseArgs() {
    ...
    const args = this.getRuntimeArgs();
  ...
  run() {
    // Show help
    if (this.args.help || this.args.files.length === 0 && ! this.args.stdin) {
      this.showHelp();
      return;
    ...
    if (this.args.watch) {
      if (this.runtime === 'bun') {
        ...
        console.error "Watch mode === currently only supported in Bun"
        this.exit(1);
      return;
...
compileStdin: async ->
  ...
  watchFiles() {
    ...
    if (typeof Bun !== 'undefined') {
      ...
      console.log("Watch mode ! yet implemented");
    } else {
