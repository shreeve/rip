# This file is running in CommonJS (in Node) so it can use import() within an async function, but not at the top level; and we can't use static import.
...
test "static import assertion", ->
  ...
  # The only file types for which import assertions are currently supported are JSON (Node) and CSS, neither of which support named exports; however there's nothing in the JavaScript grammar preventing a future supported file type from providing named exports.
  eqJS """
