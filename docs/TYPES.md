...
**Both spellings.** HTML attribute names answer in the spec spelling and the camelCase property spelling where the case fold lands on the real attribute (`maxlength`/`maxLength` — the CAMEL bridge, shared with the extends-props surface). `for:` is legal and types through `htmlFor`; `htmlFor` itself is NOT an attr-road key (its case fold is the unrelated `htmlfor` attribute). SVG names take no doubling.

**Events.** The handler casts carry the event's real function type with `target`/`currentTarget` claimed as the HOST element (RULINGS.md — the re-ruling superseding `any`-by-design): a literal `(e) ->` handler's param types contextually, a named method checks against the event's function type at the binding AND types its own declared bare param through the render walk's tag record (a method serving several hosts claims their union; an unknowable host stays `any`). A custom event name is legal DOM (src/dom.js) — it rides the string overload, its literal handler's param takes contextual `any` (quiet under `noImplicitAny`, no claim), and no did-you-mean exists for event names (a misspelling IS a legal custom event).
...
- Boolean-attribute and presence author values are truthiness positions by the roads' own semantics (`!!expr`, `if (expr)`, the `?!` scratch const) — governed, never constrained (tsgo's TS2872 fires incidentally on literal nonsense). A presence value still checks against the attribute's type at the scratch const, `| null | undefined` for the remove arm.
- The merge-road `class:` key has no generated bytes of its own (the pair dissolves into one `__clsx` call) — it rides a cover row over the value bytes.
- Selector-token words (`div#id.class`) live inside one lexer token — no per-word rows; grammar-validated, hover forgone.
- A `<=>` bind's write is typed at the CELL, not the element (the runtime coerces both directions).
...
- A 2+-parameter literal handler keeps `as any` — arity forgiveness is legal Rip by design (v3 rejects it).
