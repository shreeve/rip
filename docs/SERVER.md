...
- browser-safe non-core package sources and package-resolution metadata;
...
The versioned browser runtime at `/@rip/rip.js` already contains the complete
compiled `@rip-lang/app` package. A bundle never repeats that stable framework
source. Other browser-safe packages travel only when an authored module
imports them.
...
Bundle assembly compiles every authored or non-core package module that ships,
resolves browser packages, and materializes allowed schema projections before
publication. The embedded `@rip-lang/app` package was compiled when `rip.js`
was built. Invalid UTF-8 Rip
...
`rash(bytes)` is the Rip hash operation: SHA-256 over the exact content,
encoded as the first six unpadded Base64URL characters with `-` folded into
`_`. The resulting value and the public manifest and ding field are `hash`.
