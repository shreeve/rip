...
if require.main is module
  ...
  showStats = (generator) ->
    ...
    Symbols:
    • Total Symbols: #{Object.keys(generator.symbols_ || {}).length}
    • Terminals: #{Object.keys(generator.terminals_ || {}).length}
    • Nonterminals: #{Object.keys(generator.nonterminals || {}).length}

    Productions:
    • Total Productions: #{generator.productions?.length || 0}
    • Average Production Length: #{if generator.productions?.length then (generator.productions.reduce(((sum, p) -> sum + p.handle.length), 0) / generator.productions.length).toFixed(1) else 0}

    Parser States:
    • Total States: #{generator.states?.length || 0}
    • States with Conflicts: #{generator.conflictStates?.length || 0}

    Parse Table:
    • Actions: #{Object.keys(generator.stateTable || {}).length}
    • Default Actions: #{Object.keys(generator.defaultActions || {}).length}
    """
