...
LALRGenerator.prototype.generateTableCode = function(table) {
    ...
    // Optimize repeated objects
    moduleCode = moduleCode.replace(/\{\d+:[^\}]+,\d+:[^\}]+\}/g, function(object) {
        var keys = {}, maxKeyCount = 0, frequentValue;
        var keyValueMatcher = /(\d+):([^:]+)(?=,\d+:|\})/g;
        var keyValue;

        while ((keyValue = keyValueMatcher.exec(object))) {
            var key = keyValue[1], value = keyValue[2];
            var keyCount = 1;
        ...
        if (maxKeyCount > 1) {
            ...
            for (var value in keys) {
                if (value !== frequentValue) {
                    for (var k = keys[value], i = 0; i < k.length; i++) {
...
LALRGenerator.prototype.createObjectCode = 'o=function(k,v,o,l){' +
    'for(o=o||{},l=k.length;l--;o[k[l]]=v);' +
    'return o}';

LALRGenerator.prototype.createVariable = function() {
    var id = this.nextVariableId++;
    var name = '$V';
    var tokens = '0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ_$';

    do {
        name += tokens[id % tokens.length];
        id = Math.floor(id / tokens.length);
    } while (id !== 0);

    return name;
};

LALRGenerator.prototype.nextVariableId = 0;
