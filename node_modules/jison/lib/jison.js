...
//
// Key canonical renamings applied:
// - go() → gotoState() - Navigate through parser states
// - go_() → gotoEncoded() - Specialized GOTO for lookahead computation
// - goPath() → gotoStateWithPath() - GOTO with path recording
// - goes → handleToSymbols - Maps production handles to generating symbols
// - nullableSets() → computeNullableSets() - NULLABLE set computation
// - firstSets() → computeFirstSets() - FIRST set computation
// - followSets() → computeFollowSets() - FOLLOW set computation
// - nullable() → isNullable() - NULLABLE predicate
//
...
// Represents an LR item (production with dot position and lookahead)
...
function ItemSet() {
    ...
    this.length = 0;
    this.reductions = [];
    this.handleToSymbols = {}; // Maps production handles to generating symbols (canonical: goes)
    this.edges = {};
    this.shifts = false;
...
// Merge another ItemSet or array into this ItemSet
...
// Check if this ItemSet contains a specific item
...
// Unified Language class for LALR(1) parser generation
...
Language.prototype.gotoEncoded = function(stateId, symbolSequence) {
...
Language.prototype.computeNullableSets = function() {
    var nonterminals = this.nonterminals;
    var self = this;
    var cont = true;
...
Language.prototype.isNullable = function(symbol) {
...
Language.prototype.computeFirstSets = function() {
    var productions = this.productions;
    var nonterminals = this.nonterminals;
    var self = this;
    var cont = true;
...
Language.prototype.computeFollowSets = function() {
    var productions = this.productions;
    var nonterminals = this.nonterminals;
    var self = this;
    var cont = true;
...
// Navigate through parser states following a symbol sequence (canonical: GOTO function)
Language.prototype.gotoState = function(startState, symbolSequence) {
    var currentState = parseInt(startState, 10);
    for (var i = 0; i < symbolSequence.length; i++) {
        currentState = this.states[currentState].edges[symbolSequence[i]] || currentState;
    }
    return currentState;
};

// Navigate through parser states and record the path taken (canonical: GOTO with path tracking)
Language.prototype.gotoStateWithPath = function(startState, symbolSequence) {
    var currentState = parseInt(startState, 10);
    var path = [];
    for (var i = 0; i < symbolSequence.length; i++) {
        var transition = symbolSequence[i] ? currentState + ":" + symbolSequence[i] : '';
        if (transition) this.lookahead.nterms_[transition] = currentState;
        path.push(transition);
        currentState = this.states[currentState].edges[symbolSequence[i]] || currentState;
        this.terms_[transition] = symbolSequence[i];
    }
