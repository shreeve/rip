...
// Helper function to merge arrays (union without duplicates)
...
class Generator {
            ...
            productions.forEach((production, k) => {
                ...
                for (let i = 0, t; t = production.handle[i]; ++i) {
                    ...
                    if (ctx) {
                        ...
                        var bool = !ctx || q === parseInt(this.nterms_[t], 10);
    ...
    generateModuleExpr() {
        ...
        out += [
            ...
            "table: "          + JSON.stringify(this.table).replaceAll('"', ''),
            "defaultActions: " + JSON.stringify(this.defaultActions).replaceAll('"', ''),
...
var generator = Generator.prototype;
...
// The trace method is called for recoverable parse errors. By default, it does
// nothing, but you can override it to log or handle recoverable errors as needed.
...
generator.first = function first (symbol) {
    // epsilon
    if (!symbol) return [];
    if (Array.isArray(symbol)) {
      let firsts = [];
      for (let i = 0; i < symbol.length; i++) {
        let part = this.first(symbol[i]);
        firsts = unionArrays(firsts, part);
        if (!this.nullable(symbol[i])) break;
      }
      return firsts;
    } else if (this.terminals && this.terminals.indexOf(symbol) !== -1) {
      return [symbol];
...
generator.firstSets = function firstSets () {
    const productions = this.productions;
    const nonterminals = this.nonterminals;
    let cont = true;
    ...
    while (cont) {
      cont = false;
      productions.forEach((production, k) => {
        let firsts = this.first(production.handle);
        let oldcount = nonterminals[production.symbol].first.length;
        unionArrays(nonterminals[production.symbol].first, firsts);
        if (oldcount !== nonterminals[production.symbol].first.length) {
          cont = true;
        }
      });
    }
...
generator.nullableSets = function nullableSets () {
...
generator.nullable = function nullable (symbol) {
    // epsilon
    if (symbol === '') {
        return true;
    // RHS
...
generator.buildTable = function buildTable () {
    this.states = this.canonicalCollection();
    this.table = this.parseTable(this.states);
    this.defaultActions = findDefaults(this.table);
};

// LR parser item representing a production with a dot position
generator.Item = class Item {
...
// Set of LR parser items with efficient lookup
generator.ItemSet = class ItemSet {
    constructor() {
    ...
    concat(set) {
        ...
        const items = set.items ? set.items() : set;
    ...
    valueOf() {
        ...
        const v = Array.from(this.map.keys()).sort().join('|');
...
generator.closureOperation = function closureOperation (itemSet /*, closureSet*/) {
...
generator.gotoOperation = function gotoOperation (itemSet, symbol) {
...
generator.canonicalCollection = function canonicalCollection () {
    var item1 = new this.Item(this.productions[0], 0, [this.EOF]);
    var firstSet = new this.ItemSet();
    firstSet.push(item1);
    var firstState = this.closureOperation(firstSet),
        states = [firstState],
        marked = 0,
        self = this,
        itemSet;
...
generator.canonicalCollectionInsert = function canonicalCollectionInsert (symbol, itemSet, states, stateNum) {
...
var NONASSOC = 0;
generator.parseTable = function parseTable (itemSets) {
    var states = [],
        nonterminals = this.nonterminals,
        ...
        itemSet.reductions.forEach(function (item, j) {
            ...
            terminals.forEach(function (stackSymbol) {
                ...
                if (action || action && action.length) {
                    ...
                    if (sol.bydefault) {
                        self.conflicts++;
                        throw new Error('Grammar error: conflict in grammar - multiple actions possible when lookahead token is ' + stackSymbol + ' in state ' + k + "\n- " + printAction(sol.r, self) + "\n- " + printAction(sol.s, self));
                        // unreachable code
                    } else {
                        action = sol.action;
                    }
    ...
    if (self.conflicts > 0) {
        ...
        throw new Error("Grammar error: " + self.conflicts + " conflict(s) found in grammar." + conflictDetails);
    }
...
generator.generateModule = function generateModule (opt) {
    ...
    var out = "/* parser generated by rip-parser " + Jison.version + " */\n";
    out += (moduleName.match(/\./) ? moduleName : "var " + moduleName) + " = ";
    out += this.generateModuleExpr();
...
class LALRGenerator extends Generator {
    constructor(grammar, options) {
        ...
        var newg = this.newg = Object.assign(Object.create(Generator.prototype), {
            oldg: this,
            trace: this.trace,
            nterms_: {},
            go_: function (r, B) {
...
Jison.Generator = function Jison_Generator (g, options) {
    ...
    // Only LALR(1) parser is supported
