// Optimized Jison Parser Generator - Clean Room Implementation
...
ItemSet.prototype.forEach = function(callback) {
...
LALRGenerator.prototype.nullableSets = function() {
    var nonterminals = this.nonterminals;
    var self = this;
    var cont = true;
...
LALRGenerator.prototype.nullable = function(symbol) {
...
LALRGenerator.prototype.firstSets = function() {
    var productions = this.productions;
    var nonterminals = this.nonterminals;
    var self = this;
    var cont = true;
...
LALRGenerator.prototype.followSets = function() {
    var productions = this.productions;
    var nonterminals = this.nonterminals;
    var self = this;
    var cont = true;
...
// LR operations
LALRGenerator.prototype.closureOperation = function(itemSet) {
...
LALRGenerator.prototype.gotoOperation = function(itemSet, symbol) {
...
LALRGenerator.prototype.canonicalCollection = function() {
...
LALRGenerator.prototype.canonicalCollectionInsert = function(symbol, itemSet, states, stateNum) {
...
LALRGenerator.prototype.parseTable = function(itemSets) {
    var states = [];
    var nonterminals = this.nonterminals;
...
LALRGenerator.prototype.findDefaults = function(states) {
    var defaults = {};
...
LALRGenerator.prototype.lookAheads = function(state, item) {
    return (!!this.onDemandLookahead && !state.inadequate) ? this.terminals : item.follows;
};

LALRGenerator.prototype.go = function(p, w) {
    var q = parseInt(p, 10);
...
LALRGenerator.prototype.goPath = function(p, w) {
    var q = p; // p is already a number, no need to parseInt
    var path = [];
    var wLength = w.length;
    var states = this.states;
    var nterms = this.newg.nterms_;
    var terms = this.terms_;

    for (var i = 0; i < wLength; i++) {
        var symbol = w[i];
        var t = symbol ? q + ":" + symbol : '';
        if (t) {
            nterms[t] = q;
            terms[t] = symbol;
        }
        path.push(t);
        q = states[q].edges[symbol] || q;
    }

    return { path: path, endState: q };
};

LALRGenerator.prototype.buildNewGrammar = function() {
    var self = this;
    var newg = this.newg;
    var states = this.states;
    ...
    var newgNonterminals = newg.nonterminals;
    var newgProductions = newg.productions;
    var terms = self.terms_;
    var nterms = newg.nterms_;

        for (var i = 0; i < statesLength; i++) {
        var state = states[i];
        var stateItems = state._items || state;
        var stateLength = stateItems.length;

        for (var j = 0; j < stateLength; j++) {
            var item = stateItems[j];
            if (item.dotPosition === 0) {
                var symbol = i + ":" + item.production.symbol;
                terms[symbol] = item.production.symbol;
                nterms[symbol] = i;

                if (!newgNonterminals[symbol]) {
                    newgNonterminals[symbol] = new Nonterminal(symbol);
                }

                var pathInfo = self.goPath(i, item.production.handle);
                var p = new Production(symbol, pathInfo.path, newgProductions.length);
                newgProductions.push(p);
                newgNonterminals[symbol].productions.push(p);

                var handle = item.production.handle.join(' ');
                var endState = states[pathInfo.endState];
                var goes = endState.goes;
                if (!goes[handle]) {
                    goes[handle] = [];
                }
...
LALRGenerator.prototype.unionLookaheads = function() {
    ...
    for (var i = 0; i < statesLength; i++) {
        var state = typeof states[i] === 'number' ? self.states[states[i]] : states[i];
        var reductions = state.reductions;
        var reductionsLength = reductions.length;

        if (reductionsLength > 0) {
            for (var j = 0; j < reductionsLength; j++) {
                var item = reductions[j];
                var follows = {};
                var itemFollows = item.follows;
                var itemFollowsLength = itemFollows.length;

                // Build follows hash for faster lookup
                for (var k = 0; k < itemFollowsLength; k++) {
                    follows[itemFollows[k]] = true;
                }

                var handle = item.production.handle.join(' ');
                var goes = state.goes[handle];
                if (goes) {
                    var goesLength = goes.length;
                    for (var l = 0; l < goesLength; l++) {
                        var symbol = goes[l];
                        var nonterminal = newgNonterminals[symbol];
                        if (nonterminal) {
                            var nonterminalFollows = nonterminal.follows;
                            var nonterminalFollowsLength = nonterminalFollows.length;

                            for (var m = 0; m < nonterminalFollowsLength; m++) {
                                var followSymbol = nonterminalFollows[m];
                                var terminal = terms[followSymbol];
                                if (terminal && !follows[terminal]) {
                                    follows[terminal] = true;
                                    itemFollows.push(terminal);
                                }
                            }
                        }
                    }
