...
function LRState() {
    ...
    if (arguments.length) {
        ...
        for (var i = this.length - 1; i >= 0; i--) {
            this._hash[this._items[i].id] = true;
        }
    }
}
...
LALRGenerator.prototype.buildProductions = function(bnf, productions, nonterminals, symbols, operators) {
    ...
    var terminalList = [], terminalMap = {};
    for (var id in symbolMap) {
        if (symbolMap.hasOwnProperty(id)) {
            var sym = symbolMap[id];
            if (!nonterminals[sym]) {
                terminalList.push(sym);
                terminalMap[id] = sym;
            }
        }
    }
    ...
    this.terminals_ = terminalMap;
    this.symbols_ = symbolMap;
    this.productions_ = productionTable;
