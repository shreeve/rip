...
// - closureOperation() → closure() - CLOSURE operation
// - gotoOperation() → goto() - GOTO operation
// - canonicalCollection() → buildLRAutomaton() - LR automaton construction
// - parseTable() → buildParseTable() - Parse table construction
// - lookAheads() → getLookaheadSet() - Lookahead set retrieval
// - findDefaults() → computeDefaultActions() - Default action computation
// - buildNewGrammar() → buildAugmentedGrammar() - Augmented grammar construction
// Variables:
// - goes → handleToSymbols - Maps production handles to generating symbols
// - edges → transitions - State transitions
// - shifts → hasShifts - Shift action indicator
// - inadequate → hasConflicts - Conflict state indicator
// - terms_ → terminalMap - Terminal symbol mapping
// - nterms_ → nonterminalMap - Nonterminal symbol mapping
// - inadequateStates → conflictStates - States with conflicts
// Classes:
// - ItemSet → LRState - LR parser state
// - Language → LALRGenerator - LALR parser generator
//
...
function Item(production, dotPosition, lookaheadSet, predecessor) {
    this.production = production;
    this.dotPosition = dotPosition || 0;  // Position of • in production
    this.follows = lookaheadSet || [];    // LALR(1) lookahead symbols
    this.predecessor = predecessor;
    this.id = parseInt(production.id + 'a' + this.dotPosition, 36);
    this.markedSymbol = this.production.handle[this.dotPosition]; // Symbol after •
}
...
function LRState() {
    ...
    this.transitions = {}; // State transitions (canonical: edges)
    this.hasShifts = false; // Has shift actions (canonical: shifts)
    this.hasConflicts = false; // Has SR/RR conflicts (canonical: inadequate)
    this.hash_ = {};
...
function LALRGenerator(grammar, options) {
    ...
    this.terminals = {}; // Terminal symbols (canonical: terms)
    this.operators = {};
    this.productions = [];
    this.conflicts = 0;
    this.resolutions = [];
    ...
    this.terminalMap = {}; // Maps symbols to terminal representations (canonical: terms_)

    // Initialize lookahead state (replaces newg creation)
    this.lookahead = {
        nonterminalMap: {}, // Maps nonterminals to states (canonical: nterms_)
        nonterminals: {},
        productions: []
    };

    this.conflictStates = []; // States with conflicts (canonical: inadequateStates)
    ...
    this.table = this.buildParseTable(this.states);
    this.defaultActions = this.computeDefaultActions(this.table);
}
    ...
    while (marked !== states.length) {
        ...
        itemSet._items.forEach(function(item) {
            if (item.markedSymbol && item.markedSymbol !== self.EOF) {
                self.insertLRState(item.markedSymbol, itemSet, states, marked - 1);
            }
        });
    }
