...
// RepresentsLR items (productions with dot positions)
class Item {
...
// Main class for building parse tables and generating parsers
class Generator {
  ...
  parseTable(itemSets) {
    ...
    if (this.conflicts > 0) {
      ...
      throw new Error("Grammar conflicts: " + this.conflicts + " " + conflictDetails);
    }
  ...
  generateModuleExpr() {
    ...
    out += [
      ...
      "parseError: " + String(this.parseError || zoo.parseError),
      "parse: " + String(zoo.parse)
    ].join(",\n");
    out += "};\n";
    ...
    out += "\nfunction Parser () {\n  this.yy = {};\n}\n";
    out += "Parser.prototype = zoo;";
    out += "zoo.Parser = Parser;";
    out += "\nreturn new Parser;\n})();";
...
// Extends Generator for LALR(1) parsing
...
zoo.trace = generator.trace;
zoo.warn = generator.warn;
zoo.error = generator.error;

zoo.parseError = parseError = function parseError(str, hash) {
  if (hash.recoverable) {
    this.trace(str);
  } else {
...
zoo.parse = function parse(input) {
  let stack = [0];
...
zoo.init = function parser_init(dict) {
...
// Add Parser class for legacy compatibility
class Parser extends LALRGenerator {
  constructor(grammar, options) {
    super(grammar, options);
  }
}
...
module.exports = { Parser, Generator, LALRGenerator, Nonterminal, Production, Item, ItemSet };

var Jison = exports.Jison = exports;

Jison.version = require('../package.json').version;

Jison.Generator = function Jison_Generator(grammar, options) {
  return new LALRGenerator(grammar, Object.assign({}, grammar.options, options));
};

return function Parser(grammar, options) {
  var gen = Jison.Generator(grammar, options);
