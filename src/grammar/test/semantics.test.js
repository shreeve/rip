...
import { exprGrammar, makeLexer } from './tests.js';
...
async function loadGeneratedParser(grammar, name) {
  ...
  const file = join(mkdtempSync(join(tmpdir(), 'solar-semantics-')), `${name}.mjs`);
...
// Annotated twin with every third-string annotation opted out — proves
// annotations never change tables or parse behavior (empty semantics).
function plainExpr(grammar = exprGrammar) {
  const g = structuredClone(grammar);
  for (const prods of Object.values(g.grammar)) {
    for (const prod of prods) {
      if (prod.length >= 3 && typeof prod[2] === 'string') prod[2] = '~ plain fixture';
    }
  }
  return g;
}
...
describe('semantic side table', () => {
  ...
  test('annotations do not change tables or actions', () => {
    ...
    const without = new Generator(plainExpr());
  ...
  test('annotations do not change parse output', async () => {
    ...
    const plain = await loadGeneratedParser(plainExpr(), 'expr-plain-behavior');
