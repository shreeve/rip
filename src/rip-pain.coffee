...
class Language
  constructor: (@language = {}, opts = {}) ->
    ...
    @closureCache     = new Map() # Cache for closure computations
  ...
  loadLanguage: ->
    ...
    unless @start?
      # Look for 'Root' first, then fallback to first nonterminal
      grammarKeys = Object.keys(@language.grammar ? @language.rules)
      if grammarKeys.includes('Root')
        @start = 'Root'
      else if grammarKeys.length > 0
        @start = grammarKeys[0]
      else
        throw new Error("No start symbol found and no grammar rules defined")

    console.log "Using start symbol: #{@start}" if @debug >= DEBUG
  ...
  buildSymbols: ->
    ...
    for [name, symbol] from @symbols
      unless lhsSymbols.has(name)
        ...
        symbol.isTerminal = false  # Explicitly set to false for nonterminals
        # Remove from tokens if it was added there
        if @tokens.has(name)
          @tokens.delete(name)

    # Validate that start symbol exists and is a nonterminal
    unless @symbols.has(@start)
      throw new Error("Start symbol '#{@start}' not found in grammar")
    if @symbols.get(@start).isTerminal
      throw new Error("Start symbol '#{@start}' cannot be a terminal")
  ...
  addErrorRecoveryRules: ->
    ...
    # Conservative error recovery rules that don't create circular dependencies
    # Only add basic error handling without loops
    errorRules = [
      ['$accept', ['error']] # $accept → error (basic error fallback)
    ]
  ...
  buildStates: ->
    ...
    console.log "Total states created: #{@states.length}" if @debug >= VERBOSE
  ...
  closure: (state) ->
    ...
    @closureCache.set(coreKey, newItems) if @closureCache?
  ...
  computeLookaheads: ->
    ...
    for state in @states
      for item in state.items
        ...
        # Compute closure of goto state
        @closureWithLookahead(gotoState)

        # Analyze for propagation vs spontaneous
        for gotoItem in gotoState.items
          ...
          for la from gotoItem.lookahead
            if la == '#'
              ...
              @propagateLinks.get(fromKey)?.add(toKey) or @propagateLinks.set(fromKey, new Set([toKey]))
            else
              # Spontaneous lookahead
              targetItem.lookahead.add(la)

    # Add initial lookahead
    if @states[0]?.items[0]
      @states[0].items[0].lookahead.add('$end')

    # Phase 2: Propagate lookaheads using links
    @propagateLookaheads()
  ...
  closureWithLookahead: (state) ->
    ...
    while workList.length > 0
      ...
      newLookahead = @firstOfString(beta)
      if beta.every (sym) => @getSymbol(sym).nullable
        newLookahead.add(la) for la from item.lookahead

      rules = @symbolRules.get(nextSym) or []
      for rule in rules
        newItem = new Item(rule, 0, new Set(newLookahead))
        if state.addItem(newItem)
          workList.push(newItem)

  # Propagate using links
  propagateLookaheads: ->
    changed = true
    maxIterations = 1000
    iterationCount = 0
    while changed and iterationCount++ < maxIterations
      ...
      for [fromKey, toKeys] from @propagateLinks
        [fromStateId, fromRuleId, fromDot] = fromKey.split('-').map(Number)
        fromState = @states[fromStateId]
        fromItem = fromState.getCoreItem(fromRuleId, fromDot)
        continue unless fromItem

        for toKey in toKeys
          [toStateId, toRuleId, toDot] = toKey.split('-').map(Number)
          toState = @states[toStateId]
          toItem = toState.getCoreItem(toRuleId, toDot)
          continue unless toItem

          oldSize = toItem.lookahead.size
          toItem.lookahead.add(la) for la from fromItem.lookahead
          changed = true if toItem.lookahead.size > oldSize
...
if process.argv[1]?.includes('rip.coffee') or process.argv[1]?.includes('rip.js')
  ...
  # process.argv.push '/Users/shreeve/Data/Code/rip/src/old/coffeescript/src/grammar.coffee'
