...
class LALRGenerator
  ...
  # ============================================================================
  # Split Architecture Generation
  # ============================================================================

  generateLanguageFile: (options = {}) ->
    languageName = options.languageName or "coffeescript"
    tableCode = @_generateTableCode @stateTable

    """
    // #{languageName} language data generated by sonar 0.5.2
    // Contains language-specific parsing data for the generic parser
  ...
  generateSplitParser: (options = {}) ->
    languageName = options.languageName or "coffeescript"
    moduleName = options.moduleName or "parser"
    version = '0.5.2'

    # Use provided languageFile path or construct default path
    languageFile = options.languageFile or "./languages/#{languageName}"

    """
    /* parser generated by sonar #{version} */
    var #{moduleName} = (function(){
    var hasProp = {}.hasOwnProperty;

    // Import language-specific data from languages directory
    var lang = require('#{languageFile}');
  ...
  generateSplitArchitecture: (options = {}) ->
    languageName = options.languageName or "coffeescript"
    moduleName = options.moduleName or "parser"

    # Generate both the language file and the split parser
    languageFile = @generateLanguageFile({languageName})
    parserFile = @generateSplitParser({languageName, moduleName})

    {
      languageFile: languageFile
      parserFile: parserFile
      languageName: languageName
      languagePath: "languages/#{languageName}.js"
      parserPath: "#{moduleName}.js"
    }
