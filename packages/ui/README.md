...
- `MenuPopup` — a `<div popover="auto" role="menu" tabindex="-1">` labelled by the trigger, with a minted id or the `id` you pass. Carries `data-open` while open and `data-closed` otherwise, and `data-side` with the side it landed on. Takes `side` (`'bottom'` by default, or `'top'`, `'left'`, `'right'`), `align` (`'start'` by default, or `'center'`, `'end'`), and `sideOffset`, the distance from the trigger in pixels (`0` by default), which a flip carries to the other side. Its `style` is the placement, so `style` does not pass through, and a margin class on it has no part in the placement.
