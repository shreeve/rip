...
`Drawer` is the Dialog with a side. Its module re-exports the Dialog's, so `Drawer.Root` is `Dialog.Root` itself and `Drawer.Trigger`, `Drawer.Title`, `Drawer.Description`, and `Drawer.Close` are the Dialog parts, and a drawer's parts read from the same root a dialog's do. `Drawer.Popup` takes the `side` (`left` by default, or `right`, `top`, `bottom`; the type is `Drawer.Side`) and extends `Dialog.Popup`, so it is the `<dialog>` with everything `Dialog.Popup` carries, plus `data-side`, and a swipe: a press inside the panel that travels toward its side drags the panel along with the transition off, and releasing past half the panel's size, or at a flick's speed, closes it, while a shorter release lets the transition carry it back; a pull into the screen moves the panel under resistance that grows as a power of the pull and springs back on release, and the popup bleeds 3rem past its edge in its own background so the pull reveals panel rather than the page, which makes the popup overflow visible; a body taller than the panel scrolls in a container of its own. The pointer is captured once a drag is past the slop, so a link under a swipe is not clicked when it ends. The swipe runs for every pointer type and only while `closedby` is `any`. A press on a field, on editable content, or inside an element marked `data-swipe-ignore` is never a swipe, and with a mouse neither is a press on text, which selects as it would anywhere; a mouse swipe starts from the panel itself. While a swipe is on, the popup carries `--swipe-progress`, the travel as a fraction of the panel, which `::backdrop` inherits, so a backdrop can fade with it; a dismissing release sets `--swipe-strength`, the time the remaining travel would take at the release speed as a fraction between 0.1 and 1, so a flick shortens the exit and a gentle release leaves it whole. The demo uses both. The panel's placement and slide are the consumer's classes, as the demo shows; the popup sets `touch-action` along the other axis so content inside still scrolls. A slide built from Tailwind's `translate-x-*` or `translate-y-*` utilities needs the other axis set on the element too (`translate-y-0` for a left or right drawer, `translate-x-0` for a top or bottom one): WebKit does not substitute an unset registered property's initial value inside `@starting-style`, so without it the entry transition starts from `translate: none` and the panel appears without sliding.
