...
const sources = {
  'dom.rip': readFileSync(new URL('../email/dom.rip', import.meta.url), 'utf8'),
  'compat.rip': readFileSync(new URL('../email/compat.rip', import.meta.url), 'utf8'),
};
...
test('email package TypeScript faces and declarations are valid', () => {
  ...
  for (const [name, source] of Object.entries(sources)) {
    ...
    files[name.replace('.rip', '.ts')] = result.code;
