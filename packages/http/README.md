...
A lightweight HTTP client that wraps native `fetch` with method
shortcuts, JSON convenience, automatic error throwing, retries with
exponential backoff, timeouts, lifecycle hooks, and reusable instances.
One file of Rip, zero dependencies.

**Runtime:** browser-safe (`rip.browser: true`). One `.rip` file — in a
browser, relative paths resolve against the page origin.
...
- **Native fetch only** — no shims, no polyfills, no feature detection; the same file runs in Bun and the browser
...
automatically (an explicit content-type is not overridden).
...
retried by default. Network errors retry on the same method policy, then
...
`null` and `undefined` values in objects are automatically filtered out.
...
| Cross-platform shims | yes (browser/Node/Deno/Bun) | — (native fetch is the platform) |
...
runtimes with a modern native `fetch`, so none of that is needed.
...
http.HTTPError             # Non-2xx response error (also a named export)
http.TimeoutError          # Timeout error (also a named export)
```
...
The suite runs end-to-end against a live `Bun.serve` fixture, pinning
URL building, query encoding, JSON bodies, error throwing, retries
(status, network, Retry-After), timeout/abort semantics, hooks, and
instance defaults/merging.
