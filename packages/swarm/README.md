...
Failed tasks land in `.swarm/died/` and their errors append to
`.swarm/errors.log`. Call `retry()` in your next `setup()` to move them
back for reprocessing — only the failed tasks run, not the entire batch.
