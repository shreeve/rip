// Quote-handling pins: quoted fields that first appear beyond the 8KB
// probe sample, Labcorp-style embedded commas, writer escaping, and
...
describe('writer: escaping', () => {
  ...
  test('value with comma AND quotes gets both treatments', () => {
    expect(CSV.write([['hello, "world"', 'ok']])).toBe('"hello, ""world""",ok\n');
  });

  test('value with newline gets quoted', () => {
    expect(CSV.write([['line1\nline2', 'ok']])).toBe('"line1\nline2",ok\n');
  });

  test('plain values are not quoted', () => {
    expect(CSV.write([['hello', 'world']])).toBe('hello,world\n');
  });
});
