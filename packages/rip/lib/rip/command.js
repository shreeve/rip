...
var BANNER, LONG_FLAG, MULTI_FLAG, OPTIONAL, OptionParser, SHORT_FLAG, SWITCHES, buildRule, buildRules, compileJoin, compileOptions, compilePath, compileScript, compileStdio, findDirectoryIndex, forkNode, hidden, joinTimeout, mkdirp, normalizeArguments, notSources, optionParser, opts, outputPath, parseOptions, printLine, printTokens, printWarn, removeSource, removeSourceDir, silentUnlink, sourceCode, sources, timeLog, usage, useWinPathSep, version, wait, watch, watchDir, watchedDirs, writeJs, splice = [].splice, indexOf = [].indexOf;

// The `coffee` utility. Handles command-line compilation of Rip
...
BANNER = `Usage: coffee [options] path/to/script.rip [args]

If called without options, \`coffee\` will run your script.`;

// The list of all the valid option flags that `coffee` knows how to handle.
SWITCHES = [['--ast', 'generate an abstract syntax tree of nodes'], ['-b', '--bare', 'compile without a top-level function wrapper'], ['-c', '--compile', 'compile to JavaScript and save as .js files'], ['-e', '--eval', 'pass a string from the command line as input'], ['-h', '--help', 'display this help message'], ['-j', '--join [FILE]', 'concatenate the source Rip before compiling'], ['-m', '--map', 'generate source map and save as .js.map files'], ['-M', '--inline-map', 'generate source map and include it directly in output'], ['-n', '--nodes', 'print out the parse tree that the parser produces'], ['--nodejs [ARGS]', 'pass options directly to the "node" binary'], ['-H', '--no-header', 'suppress the "Generated by" header'], ['-o', '--output [PATH]', 'set the output path or path/filename for compiled JavaScript'], ['-p', '--print', 'print out the compiled JavaScript'], ['-s', '--stdio', 'listen for and compile scripts over stdio'], ['--tokens', 'print out the tokens that the lexer/rewriter produce'], ['-v', '--version', 'display the version number'], ['-w', '--watch', 'watch scripts for changes and rerun commands']];
...
export var buildCSOptionParser = function() {
  return new OptionParser(SWITCHES, BANNER);
};
...
replace...
    ...
    $ coffee --compile --join bundle.js -- a.rip b.rip c.rip

with...

    $ cat a.rip b.rip c.rip | coffee --compile --stdio > bundle.js
`);
  }
  ref = opts.arguments;
  results = [];
...
compileScript = function(file, input, base = null) {
  ...
  try {
    ...
    if (opts.tokens) {
      return printTokens(Rip.tokens(task.input, task.options));
    } else if (opts.nodes) {
      return printLine(Rip.nodes(task.input, task.options).toString().trim());
    } else if (opts.ast) {
      compiled = Rip.compile(task.input, task.options);
      return printLine(JSON.stringify(compiled, null, 2));
    } else if (opts.run) {
      return Rip.run(task.input, task.options);
    ...
    if (Rip.listeners('failure').length) {
      return;
    }
...
version = function() {
  return printLine(`Rip version ${Rip.VERSION}`);
};
