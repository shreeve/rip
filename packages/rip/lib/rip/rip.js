...
// Main module for tokenizing, parsing, and compiling Rip to JavaScript.
...
export var FILE_EXTENSIONS = ['.rip'];
...
withPrettyErrors = function(fn) {
  return function(code, options = {}) {
    ...
    try {
      ...
      err = error;
      if (typeof code !== 'string') { // Support `Rip.nodes(tokens)`.
        throw err;
      }
...
// Compile Rip code to JavaScript.
//
...
// object, where sourceMap is a sourcemap.rip#SourceMap object, handy for
...
export var patchStackTrace = function() {
  ...
  // Based on [michaelficarra/RipRedux](http://goo.gl/ZTx1p)
  // NodeJS / V8 have no support for transforming positions in stack traces using
  // sourceMap, so we must monkey-patch Error to display Rip source
  // positions.
    ...
    frames = (function() {
      ...
      for (i = 0, len = stack.length; i < len; i++) {
        ...
        if (frame.getFunction() === exports.run) {
          // Don't display stack frames deeper than `Rip.run`.
          break;
        }
