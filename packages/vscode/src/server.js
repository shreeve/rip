...
import { FALLBACK_LEGEND, importRecordOf, importedKindInRecord, correctSemanticTokens } from './tokens.js';
...
function mirrorFromDisk(fsPath, source) {
  ...
  cacheManifest.entries[fsPath] = {
    ...
    // What an IMPORTER needs to color its own uses of this module's names
    // (tokens.js), the one set of facts it cannot compute for itself.
    // Cheap to carry (names, no spans) and invalidated with the entry; a
    // manifest written before a field existed is purged wholesale by the
    // cacheIdentity key, which a server change already moves.
    ...importRecordOf(result),
  };
  scheduleManifestSave();
...
// tsgo's semantic-tokens legend, mirrored to the extension's own client;
// the fallback governs only when tsgo failed to start.
...
async function refresh(document) {
  ...
  const good = {
    ...
    // The names this module records for its IMPORTERS (tokens.js), which
    // cannot compute them from their own compile. An open buffer answers
...
// as: 'namespace' (`export * as name`, or an exported object of the
// module's own components), 'component', or null. Answered as
...
function importedKindOf(fromDir, specifier, name) {
  ...
  return importedKindInRecord(record, name);
}
...
// The corrections live in tokens.js; this binds them to the server's
// module records and encodes the answer for the client.
function ripSemanticTokens(ctx, data) {
  const tokens = correctSemanticTokens(ctx, data, {
    legend: semanticTokensLegend,
    importedKindOf: (specifier, name) => importedKindOf(ctx.good.dir, specifier, name),
    declaresEnum: (specifier, name) => declaresEnum(ctx.good.dir, specifier, name),
  });
