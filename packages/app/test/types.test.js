...
test.skip('app package TypeScript faces and declarations are valid (deferred: package .d.ts removed until typing pass)', () => {
  const files = {
    ...
    'consumer.ts': [
      ...
      'data.user.ensure();',
      "const userHandle = data.source('user');",
      'const handleLoading: boolean = userHandle.loading;',
      'const handleError: unknown = userHandle.error;',
      'const handleRefetched: Promise<unknown> = userHandle.refetch();',
      'const handleReset: void = userHandle.reset();',
      "const keyedHandle = data.source('order', 'o1');",
      "keyedHandle.value = { id: 'seeded' };",
      '// @ts-expect-error handle loading is read-only',
      'userHandle.loading = false;',
      'void userHandle; void handleLoading; void handleError; void handleRefetched; void handleReset; void keyedHandle;',
      ...
      "const routesManifest = buildRoutes(['routes/index.rip']);",
      "const fake = { read: () => '/', push(u: string, s: unknown) {}, replace(u: string, s: unknown) {}, go(d: number) {}, listen: (fn: () => void) => () => {}, scroll: { save: () => ({ x: 0, y: 0 }), watch: (fn: () => void) => () => {} } };",
