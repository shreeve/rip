...
`buildRoutes(files, root = 'routes')` compiles route files into a frozen,
deterministic manifest with no browser dependency:

```coffee
manifest = buildRoutes components.listAll('routes')
...
```coffee
router = createRouter
  routes: buildRoutes components.listAll('routes')
