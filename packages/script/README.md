...
chat = Script.ssh! 'user@host',
  ...
  cols: 80         # PTY columns for spawn/ssh (default: 80)
  rows: 24         # PTY rows for spawn/ssh (default: 24)
...
| `bomb` | `true`  | Throw on timeout (false = keep waiting silently) |
...
| `cols` | `80`    | PTY columns (spawn/ssh transports) |
| `rows` | `24`    | PTY rows (spawn/ssh transports) |
