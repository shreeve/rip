...
(async () => {
  ...
  const items = ast.lines[0].cmds[0].cond.pat.items;
  const can = items.filter(i=>i.type==='Class').map(i=>i.canonical);
  assert.deepEqual(can, ['PUNCT','BLANK','SPACE','CONTROL']);
  console.log('PASS');
