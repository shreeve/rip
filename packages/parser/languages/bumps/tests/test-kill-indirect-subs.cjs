...
(async () => {
  ...
  const v = ast.lines[0].cmds[0].args.items[0];
  assert.equal(v.type, 'Var');
  assert.equal(v.global, true);
  assert.equal(v.subs.length, 3);
  assert.equal(v.subs[0].type, 'Indirect');
  assert.equal(v.subs[1].type, 'String');
  assert.equal(v.subs[2].type, 'Indirect');
  console.log('PASS');
