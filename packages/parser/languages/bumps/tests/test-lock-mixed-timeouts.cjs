...
(async () => {
  ...
  const items = ast.lines[0].cmds[0].args;
  assert.equal(items.length, 3);
  assert.equal(items[0].timeout.type, 'Number');
  assert.equal(items[1].timeout, null);
  assert.ok(items[2].timeout && typeof items[2].timeout === 'object');
  console.log('PASS');
