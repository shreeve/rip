...
(async () => {
  ...
  assert.equal(cmd.op, 'USE');
  assert.equal(cmd.args.type, 'ArgsDEVICE');
  assert.equal(cmd.args.specs.length, 3);
  assert.equal(cmd.args.specs[0].params.length, 1);
  assert.equal(cmd.args.specs[1].params.length, 0);
  assert.equal(cmd.args.specs[2].params.length, 2);
  console.log('PASS');
