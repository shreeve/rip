...
(async () => {
  ...
  assert.equal(cmd.op, 'OPEN');
  assert.equal(cmd.args.type, 'ArgsDEVICE');
  assert.equal(cmd.args.specs.length, 2);
  assert.equal(cmd.args.specs[0].params.length, 2);
  assert.equal(cmd.args.specs[1].params.length, 1);
  console.log('PASS');
