...
(async () => {
  ...
  const tgt = ast.lines[0].cmds[0].args.targets[0];
  assert.equal(tgt.label, 'L');
  assert.equal(tgt.offset, 1);
  assert.equal(tgt.routine, 'R');
  assert.equal(tgt.args.length, 2);
  console.log('PASS');
