...
(async () => {
  ...
  const m = ast.lines[0].cmds[0].args[0];
  assert.equal(m.type, 'Merge');
  assert.equal(m.target.type, 'Var');
  assert.equal(m.target.global, true);
  assert.equal(m.target.subs.length, 2);
  assert.equal(m.source.type, 'Var');
  assert.equal(m.source.global, true);
  assert.equal(m.source.subs.length, 2);
  console.log('PASS');
