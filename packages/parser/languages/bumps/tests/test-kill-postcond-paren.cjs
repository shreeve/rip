...
(async () => {
  ...
  assert.equal(cmd.op, 'KILL');
  assert.ok(cmd.pc);
  const items = cmd.args.items;
  assert.equal(items.length, 3);
  assert.equal(items[0].type, 'Var');
  assert.equal(items[1].type, 'Var');
  assert.equal(items[1].global, true);
  assert.equal(items[2].type, 'Indirect');
  console.log('PASS');
