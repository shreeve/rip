...
(async () => {
  ...
  const tgt = ast.lines[0].cmds[0].args[0];
  assert.equal(tgt.type, 'EntryRef');
  assert.equal(tgt.label, 'TAG');
  assert.equal(tgt.routine, 'ROU');
  assert.equal(tgt.offset, -2);
  console.log('PASS');
