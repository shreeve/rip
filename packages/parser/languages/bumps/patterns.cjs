function parsePattern(input) {
  ...
  function parseAtom(){ const count=parseCount(); if(eof()) return null; const ch=peek(); if(ch==='"'){ const str=readString(); return applyCount({type:'String',value:str},count); }
    ...
    if(/^[A-Za-z]$/.test(ch)){ next(); return applyCount({type:'Class',name:ch.toUpperCase()},count); }
