...
# Rip Site - @rip-lang/site

> **Rip Site — concise routes, smart responses, validated input, safe hot reload,
> and disposable Bun workers behind Caddy and Janus (App + API)**

Rip Site has four surfaces that share one contract:

- `@rip-lang/site` is the framework API source imports: routes, response
...
```bash
bun add @rip-lang/site
```

Create `index.rip`:

```coffee
import { get, post, read, error, prefix, start } from '@rip-lang/site'
...
```coffee
import { use, session } from '@rip-lang/site'
import {
  cors, csrf, htmlJson, secureHeaders, sessions, timeout
} from '@rip-lang/site/middleware'
...
Rip Site owns publication construction and commit. Its focused suite pins
