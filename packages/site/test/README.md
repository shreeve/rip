...
These fixtures earn the Rip Site implementation one observable capability at
...
10. `appliance` — the private Rip Agent plus `rip sites` / `rip edge` lifecycle.
11. `janus` — the released Janus data plane and control plane.

Code enters `@rip-lang/site` only when one of these fixtures demonstrates why
